...
creme.ActivityCalendar = creme.component.Component.sub({
    ...
    defaultSourceId: function(id) {
        return this.prop('defaultSourceId', id);
    },
    ...
    eventSelectUrl: function(url) {
        return this.prop('eventSelectUrl', url);
    },
    ...
    selectedSourceIds: function(ids) {
        if (ids === undefined) {
            return Array.from(
                Object.isFunc(this._selectedSourceIds) ? this._selectedSourceIds(this) : this._selectedSourceIds
            );
        }

        this._selectedSourceIds = Object.isFunc(ids) ? ids : new Set(ids || []);
        this.refetchEvents();
        return this;
    },
    ...
    selectSourceIds: function(ids) {
        ...
        // TODO: remove the mechanism of selection view. Just use a list of ids
        ids = new Set(ids || []);
        ...
        var query = this._query({
            ...
            if (!Object.isFunc(self._selectedSourceIds)) {
                self._selectedSourceIds = self._selectedSourceIds.union(ids);
            }
    ...
    unselectSourceIds: function(ids) {
        // TODO: remove the mechanism of selection view. Just use a list of ids
        ids = new Set(ids || []);
        ...
        var query = this._query({
            ...
            remove: Array.from(ids)
        }).onDone(function() {
            var calendar = self.fullCalendar();

            if (!Object.isFunc(self._selectedSourceIds)) {
                self._selectedSourceIds = self._selectedSourceIds.difference(ids);
            }

            if (calendar) {
                var selectedIds = new Set(self.selectedSourceIds());

                var removedEvents = calendar.getEvents().filter(function(event) {
                    return !selectedIds.has(String(event.extendedProps.calendar));
                });
    ...
    */
    ...
    _onCalendarExternalEventUpdate: function(calendar, info) {
        ...
        var range = new creme.CalendarEventRange({
            start: this._toMoment(info.date, calendar),
    ...
    _onCalendarEventUpdate: function(calendar, info) {
        var self = this;
        var range = new creme.CalendarEventRange({
            start: this._toMoment(info.event.start, calendar),
            end: this._toMoment(info.event.end, calendar),
            duration: calendar.getOption('defaultTimedEventDuration'),
    ...
    _onCalendarEventFetch: function(calendar, info, successCb, failureCb) {
        var self = this;
        var range = new creme.CalendarEventRange({
            start: this._toMoment(info.start, calendar),
            end: this._toMoment(info.end, calendar)
        });
    ...
    _eventTimeSlotCount: function(event, calendar) {
        if (event.end) {
            var start = this._toMoment(event.start, calendar);
            var end = this._toMoment(event.end, calendar);
    ...
    _postRenderCalendarView: function(calendar, info) {
        ...
        if (this.showWeekNumber()) {
            ...
            var start = this._toMoment(view.activeStart, calendar);
    ...
    _renderWeekNumber: function(calendar, info) {
        return this._toMoment(info.date, calendar).format('[S] W');
    },

    _onCalendarEventOverlap: function(calendar, still, moving) {
        var overlaps = this._props.allowEventOverlaps;

        if (Object.isFunc(this._props.allowEventOverlaps)) {
            return overlaps(calendar, still, moving);
        } else {
    ...
    _setupCalendarHandlers: function(calendar) {
        ...
        calendar.on('datesSet', function(info) {
            ...
            self.trigger('range-select', {
                activityCalendar: self,
                view: info.view.type,
                start: self._toMoment(info.view.activeStart, calendar),
                end: self._toMoment(info.view.activeEnd, calendar),
                calEvent: info
            });
        });
