...
QUnit.parametrize('creme.CalendarEventRange (dayRange)', [
    ...
    deepEqual(range.dayRange(), expected);
});
...
QUnit.parametrize('creme.CalendarEventRange (isWithinBusinessHours)', [
    ...
    equal(range.isWithinBusinessHours({
...
QUnit.test('creme.ActivityCalendar (empty)', function(assert) {
    ...
    equal(true, controller.allowEventOvertime());
    equal(true, controller.allowEventAnyDay());
    equal(true, controller.allowSelection());
...
QUnit.test('creme.ActivityCalendar (options)', function(assert) {
    ...
    equal(false, controller.allowEventOvertime());
    equal(false, controller.allowEventAnyDay());
    equal(false, controller.allowSelection());
...
QUnit.parameterize('creme.ActivityCalendar.selectAllow', [
    ...
    equal(this.simulateCalendarSelectAllow(controller, {
        id: '3',
        start: newEventStart.toDate(),
        end: newEventEnd.toDate(),
        allDay: false
...
QUnit.parameterize('creme.ActivityCalendar.allowSelection (function)', [
    ...
    equal(0, fakeAllowSelection.count());
