...
class ActivityBricksTestCase(BrickTestCaseMixin, _ActivitiesTestCase):
    ...
    def test_user_calendars(self):
        # user = self.login()
        user = self.login(is_superuser=False, allowed_apps=['activities'])
    ...
    def test_user_calendars__no_app_perm(self):
        self.login(is_superuser=False, allowed_apps=['persons'])  # Not 'activities'

        response = self.assertGET200(reverse('creme_config__user_settings'))
        self.assertNoBrick(
            self.get_html_tree(response.content), brick_id=UserCalendarsBrick.id_,
        )
