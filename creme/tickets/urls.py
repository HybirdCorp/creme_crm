...
if not tickets.ticket_model_is_custom():
    ...
    urlpatterns += [
        ...
        url(r'^ticket/add[/]?$',                     ticket.TicketCreation.as_view(), name='tickets__create_ticket'),
        # url(r'^ticket/edit/(?P<ticket_id>\d+)[/]?$', ticket.edit,       name='tickets__edit_ticket'),
        url(r'^ticket/edit/(?P<ticket_id>\d+)[/]?$', ticket.TicketEdition.as_view(), name='tickets__edit_ticket'),
...
if not tickets.tickettemplate_model_is_custom():
    ...
    urlpatterns += [
        ...
        url(r'^template/edit/(?P<template_id>\d+)[/]?$', template.TicketTemplateEdition.as_view(), name='tickets__edit_template'),
        # url(r'^template/(?P<template_id>\d+)[/]?$',      template.detailview, name='tickets__view_template'),
        url(r'^template/(?P<template_id>\d+)[/]?$',      template.TicketTemplateDetail.as_view(), name='tickets__view_template'),
    ]
