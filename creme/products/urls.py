...
if not products.product_model_is_custom():
    urlpatterns += [
        ...
        url(r'^product/edit/(?P<product_id>\d+)[/]?$', product.ProductEdition.as_view(), name='products__edit_product'),
...
if not products.service_model_is_custom():
    urlpatterns += [
        ...
        url(r'^service/edit/(?P<service_id>\d+)[/]?$', service.ServiceEdition.as_view(), name='products__edit_service'),
