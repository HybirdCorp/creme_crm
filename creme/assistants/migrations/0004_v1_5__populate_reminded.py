...
class Migration(DataMigration):
    def forwards(self, orm):
        def _update_reminded(name):
            ctypes = orm['contenttypes.ContentType'].objects.filter(app_label='assistants',
                                                                    model=name.lower(),
                                                                   )[:1]

            if ctypes:
                orm['assistants.%s' % name].objects \
                                           .filter(id__in=orm['creme_core.DateReminder'].objects
                                                                                        .filter(model_content_type=ctypes[0])
                                                                                        .values_list('model_id', flat=True)
                                                  ) \
                                           .update(reminded=True)

        _update_reminded('ToDo')
        _update_reminded('Alert')

    def backwards(self, orm):
        for name in ('ToDo', 'Alert'):
            orm['assistants.%s' % name].objects.update(reminded=False)
    ...
    complete_apps = ['creme_core', 'assistants']
    symmetrical = True
