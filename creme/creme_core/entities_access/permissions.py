...
#COMMENTÉ LE 25/05/2010
#def _get_modelform_class_for(module_name, form_name):
#    """Return the class for given module and modelform name"""
#    m = __import__(module_name)
#    for i in module_name.split(".")[1:]:
#        m = getattr(m, i)
#
#    return getattr(m, form_name)
#
#def user_has_permissions_for_an_contenttype(request, module_name, form_name, object_id=None):
...
#        return False
#
#    form_obj = _get_modelform_class_for(module_name, form_name)
#    content_type = ContentType.objects.get_for_model(form_obj._meta.model)
#    debug("user_has_permissions_for_an_object.ContentType: %s", content_type)
#
#    Droit_ses_Fiches = Role.droits_entity_type.filter(content_type=content_type,
#                type_droit__name=constants.DROIT_TYPE_DROIT_MODIFIER,
...
#    return False
#
#def user_has_permissions_for_an_object(request, module_name, form_name, object_id=None):
#    if request.user.is_superuser:
#        return True
#
#    try:
...
#    #Role = request.user.get_profile().creme_role
#    form_obj = _get_modelform_class_for(module_name, form_name)
#    cremeEntity = CremeEntity.objects.get(pk=object_id)
#    content_type = ContentType.objects.get_for_model(form_obj._meta.model)
#    debug("user_has_permissions_for_an_object.ContentType: %s", content_type)
#
#    Droit_content_type = Role.droits_entity_type.filter(content_type=content_type,
#                    type_droit__name=constants.DROIT_TYPE_DROIT_MODIFIER)
#
#    if not Droit_content_type:
#        return False
#
#    Droit_ses_Fiches = Droit_content_type.filter(content_type=content_type,
#                type_droit__name=constants.DROIT_TYPE_DROIT_MODIFIER,
#                type_ensemble_fiche__name=constants.DROIT_TEF_SES_FICHES)
#
#    if Droit_ses_Fiches:
#        if cremeEntity.user == request.user:
#            return True
#
#    #TODO: useful !??
#    Droit_toutes_les_Fiches = Droit_content_type.filter(content_type=content_type,
#            type_droit__name=constants.DROIT_TYPE_DROIT_MODIFIER,
#            type_ensemble_fiche__name=constants.DROIT_TEF_TOUTES_LES_FICHES)
#
#    return False
