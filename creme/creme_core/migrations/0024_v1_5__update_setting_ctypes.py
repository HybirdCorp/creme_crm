...
class Migration(DataMigration):
    depends_on = (
        ('creme_config', '0002_v1_5__del_settingkey__new_settingvalue.py'),
    )

    def forwards(self, orm):
        orm['contenttypes.ContentType'].objects.filter(app_label='creme_config', model='settingvalue') \
                                               .update(app_label='creme_core')
        orm['contenttypes.ContentType'].objects.filter(app_label='creme_config', model='settingkey') \
                                               .delete()

    def backwards(self, orm):
        orm['contenttypes.ContentType'].objects.filter(app_label='creme_core', model='settingvalue') \
                                               .update(app_label='creme_config')
        orm['contenttypes.ContentType'].objects.create(app_label='creme_config',
                                                       model='settingkey',
                                                       name='setting key',
                                                      )
