...
class Migration(DataMigration):
    def forwards(self, orm):
        users = orm['auth.User'].objects.filter(role=None, is_superuser=False)

        if not users:
            return

        defcreds_objs = orm['creme_core.EntityCredentials'].objects.filter(entity__isnull=True)[:1]
        defcreds = defcreds_objs[0].value if defcreds_objs else NO_CRED

        role = orm['creme_core.UserRole'].objects.create(name='DEFAULT_ROLE_CREATED_BY_DATA_MIGRATION')

        value = CRED_NONE
        for str_creds, bin_creds in CRED_MATCHES:
            if str_creds in defcreds:
                value |= bin_creds

        orm['creme_core.SetCredentials'].objects.create(role=role, value=value, set_type=ESET_ALL)
    ...
    def backwards(self, orm):
        # We cannot be sure if a role has been created by this script or the user
        # It is not annoying that all user have a role anyway.
        pass
