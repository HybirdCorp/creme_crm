...
class WorkflowRegistry:
    ...
    def action_source_formfields(self,
        ...
        for sub_source in root_sources:
            for source_cls in self._action_source_classes.values():
                field = source_cls.composed_config_formfield(sub_source=sub_source, user=user)
                if field is not None:
        ...
        for action_class in action_classes:
            ...
            if not type_id:
                raise self.RegistrationError(
                    f'This action class has an empty ID: {action_class}'
                )

            if set_action(type_id, action_class) is not action_class:
                raise self.RegistrationError(
                    f'This action class uses a duplicated ID: {action_class}'
                )
    ...
    def register_action_sources(self,
        ...
        for src_class in source_classes:
            type_id = src_class.type_id
            if not type_id:
                raise self.RegistrationError(
                    f'This source class has an empty ID: {src_class}'
                )

            if set_source(type_id, src_class) is not src_class:
                raise self.RegistrationError(
                    f'This source class uses a duplicated ID: {src_class}'
                )
        ...
        for action_class in action_classes:
            try:
                ...
                raise self.UnRegistrationError(
                    f'This action class is not registered: {action_class}'
                ) from e
    ...
    def unregister_action_sources(self,
        ...
        for src_class in source_classes:
            try:
                ...
                raise self.UnRegistrationError(
                    f'This source class is not registered: {src_class}'
                ) from e
    ...
    def register_triggers(self,
        ...
        for trigger_class in trigger_classes:
            type_id = trigger_class.type_id
            if not type_id:
                raise self.RegistrationError(
                    f'This trigger class has an empty ID: {trigger_class}'
                )

            if set_trigger(type_id, trigger_class) is not trigger_class:
                raise self.RegistrationError(
                    f'This trigger class uses a duplicated ID: {trigger_class}'
                )
    ...
    def unregister_triggers(self,
        ...
        for trigger_class in trigger_classes:
            try:
                ...
                raise self.UnRegistrationError(
                    f'This trigger class is not registered: {trigger_class}'
                ) from e
