...
class WorkflowConditions:
    ...
    def from_dicts(cls, data: list[dict], registry: WorkflowRegistry) -> WorkflowConditions:
        ...
        for d in data:
            source = registry.build_action_source(d['entity'])
    ...
    # TODO: OR VS AND
    def accept(self, user: CremeUser, context: dict) -> bool:
        for source_conditions in self._conditions_per_source:
            ...
            for condition in source_conditions['conditions']:
                if not condition.accept(entity=entity, user=user):
                    return False
    ...
    def descriptions(self, user: CremeUser) -> Iterator[str]:
        for source_conditions in self._conditions_per_source:
            source: WorkflowActionSource = source_conditions['entity']
...
class WorkflowActionSource:
    ...
    @classmethod
    def config_formfield_kind_id(cls, sub_source: WorkflowActionSource | None = None) -> str:
    ...
    @property
    def sub_source(self) -> WorkflowActionSource | None:
