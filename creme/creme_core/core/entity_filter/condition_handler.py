...
class RegularFieldConditionHandler(OperatorConditionHandlerMixin,
    ...
    def accept(self, *, entity, user):
        ...
        if isinstance(last_field, ForeignKey):
            ...
            if not isinstance(operator, operators.IsEmptyOperator):
                ...
                values = [
                    obj.pk for obj in last_field.related_model.objects
                                                              .get_by_portable_keys(values)
                ]
        # TODO: factorise
        ...
        if values is None:
            ...
            if (
                ...
                try:
                    ...
                    instances = [
                        *related_model._default_manager.get_by_portable_keys(portable_keys),
                    ]
    ...
    def get_q(self, user):
        ...
        if isinstance(last_field, BooleanField):
            ...
            if not isinstance(operator, operators.IsEmptyOperator):
                values = [
                    obj.pk for obj in last_field.related_model
                                                .objects
                                                .get_by_portable_keys(values)
                ]
...
class CustomFieldConditionHandler(OperatorConditionHandlerMixin,
    ...
    def description(self, user):
        ...
        values = self._verbose_values
        if values is None:
            # TODO: improve; see RegularFieldConditionHandler.description
            if (
                ...
                try:
                    ...
                    values = [*CustomFieldEnumValue.objects.filter(uuid__in=self._values)]
                # except ValueError:
