...
QUnit.test('creme.widget.template (template:no keys)', function(assert) {
    ...
    assert.equal(result, 'template without key');
...
QUnit.test('creme.object.deferred (finished)', function(assert) {
    ...
    assert.deepEqual(element.data('deferred__overlay'), deferred);
    assert.deepEqual(result, []);
    ...
    setTimeout(function() {
        assert.equal(element.data('deferred__overlay'), undefined);
...
QUnit.test('creme.object.deferred (canceled)', function(assert) {
    ...
    assert.deepEqual(deferred, element.data('deferred__overlay'));
    ...
    setTimeout(function() {
        assert.deepEqual(deferred, element.data('deferred__overlay'));
        assert.deepEqual(result, []);
        assert.deepEqual(deferred, creme.object.deferred_cancel(element, 'overlay'));
...
QUnit.test('creme.object.deferred (restarted)', function(assert) {
    ...
    setTimeout(function() {
        ...
        assert.notEqual(element.data('deferred__overlay'), undefined, 'replaced');
        assert.deepEqual(result, []);
        start();
    }, 90);

    setTimeout(function() {
        assert.notEqual(element.data('deferred__overlay'), undefined, 'replaced after first elapsed');
        assert.deepEqual(result, []);
        start();
    }, 300);

    setTimeout(function() {
        assert.equal(element.data('deferred__overlay'), undefined, 'replaced done');
        assert.deepEqual(result, ['done restarted']);
        start();
    }, 700);
});
*/
...
QUnit.test('creme.object.build_callback (function)', function(assert) {
    ...
    assert.equal(cb, creme.object.build_callback(cb));
    assert.equal(cb, creme.object.build_callback(cb, ['arg1', 'arg2']));
});
...
QUnit.test('creme.object.build_callback (valid script, parameters)', function(assert) {
    ...
    assert.equal(typeof cb, 'function');
    assert.equal(cb(0, 0), 0);
    assert.equal(cb(5, 2), 5 * 2);
    assert.equal(cb(4.56, 2), 4.56 * 2);

    cb = creme.object.build_callback('if (arg1 > arg2) {return arg1;} else {return arg2;}', ['arg1', 'arg2']);
    assert.equal(typeof cb, 'function');
    assert.equal(cb(0, 0), 0);
    assert.equal(cb(5, 2), 5);
    assert.equal(cb(4.56, 445), 445);
});
*/
}(jQuery));
