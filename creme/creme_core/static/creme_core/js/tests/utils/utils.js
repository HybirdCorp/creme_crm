...
QUnit.test('creme.utils.JSON.encode', function(assert) {
    ...
    assert.equal(encoder('test'), '"test"');
    assert.equal(encoder(12), '12');
    assert.equal(encoder(['a', 12, 'c', null, undefined]), '["a",12,"c",null,null]');
    assert.equal(encoder({'a': ['a', 'b', 150],
                   'b': 'test',
                   'c': 12
                  }), '{"a":["a","b",150],"b":"test","c":12}');
    */
...
QUnit.test('creme.utils.JSON.decode (invalid or null, default)', function(assert) {
    ...
    assert.equal(decoder('{"a\':1}'), 'default');
    assert.equal(decoder('{"a":1,}'), 'default');
    assert.equal(decoder('{a:1}'), 'default');
    assert.equal(decoder(null), 'default');

    assert.equal(decoder('{"a\':1}', 'fail'), 'fail');
    assert.equal(decoder('{"a":1,}', 'fail'), 'fail');
    assert.equal(decoder('{a:1}', 'fail'), 'fail');
    assert.equal(decoder(null, 'fail'), 'fail');
    */
});
...
QUnit.test('creme.utils.JSON.decode (valid)', function(assert) {
    ...
    assert.deepEqual(decoder('{"a":1, "b":true, "c":[1, 2, 3]}'), {a: 1, b: true, c: [1, 2, 3]});
    */
});
