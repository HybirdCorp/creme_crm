...
QUnit.parameterize('creme.ajax.jqueryFormSubmit (url)', [
    ...
    assert.equal(form.attr('action'), expected.action);
    assert.equal(faker.count(), 1);

    var call = faker.calls()[0][0];
    assert.equal(call.iframe, false);
});
...
QUnit.parameterize('creme.ajax.jqueryFormSubmit (options)', [
    ...
    var call = faker.calls()[0][0];
    assert.equal(call.iframe, false);
    assert.deepEqual(call.headers, expected.headers);
});
...
QUnit.test('creme.ajax.jqueryFormSubmit (error callback)', function(assert) {
    ...
    assert.ok(Object.isFunc(submitCall.success));
    assert.ok(Object.isFunc(submitCall.error));
