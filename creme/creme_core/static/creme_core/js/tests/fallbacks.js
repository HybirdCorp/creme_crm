...
if (jQuery === undefined) {
    QUnit.test('fallbacks.Array.every', function(assert) {
        assert.equal(typeof Array.prototype.every, 'function');
        assert.equal(typeof [].every, 'function');

        // all elements >= 15
        assert.equal([22, 72, 16, 99, 254].every(function(element, index, array) {
                                              return element >= 15;
                                          }), true);

        // first element is < 15
        assert.equal([12, 72, 16, 99, 254].every(function(element, index, array) {
                                              return element >= 15;
                                          }), false);
    });

    QUnit.test('fallbacks.Array.filter', function(assert) {
        assert.equal(typeof Array.prototype.filter, 'function');
        assert.equal(typeof [].filter, 'function');

        assert.deepEqual([12, 5, 8, 1, 44].filter(function(element, index, array) {
                                               return element >= 10;
                                           }), [12, 44]);
    });
/*
    QUnit.test('fallbacks.Array.getRange', function(assert) {
        assert.equal(typeof Array.prototype.getRange, 'function');
        assert.equal(typeof [].getRange, 'function');

        assert.deepEqual([1, 2, 1, 4, 5, 4].getRange(2, 4), [1, 4, 5]);
    });


    QUnit.test('fallbacks.Array.inArray', function(assert) {
        assert.equal(typeof Array.prototype.inArray, 'function');
        assert.equal(typeof [].inArray, 'function');

        assert.equal([12, 5, 8, 5, 44].inArray(5), true);
        assert.equal([12, 5, 8, 5, 44].inArray(58), false);
    });

    QUnit.test('fallbacks.Array.insertAt', function(assert) {
        assert.equal(typeof Array.prototype.insertAt, 'function');
        assert.equal(typeof [].insertAt, 'function');

        assert.deepEqual(['dog', 'cat', 'horse'].insertAt(2, 'mouse'), ['dog', 'cat', 'mouse', 'horse']);
    });

    QUnit.test('fallbacks.Array.removeAt', function(assert) {
        assert.equal(typeof Array.prototype.removeAt, 'function');
        assert.equal(typeof [].removeAt, 'function');

        assert.deepEqual(['dog', 'cat', 'mouse', 'horse'].removeAt(2), ['dog', 'cat', 'horse']);
    });
*/
    QUnit.test('fallbacks.Array.some', function(assert) {
        assert.equal(typeof Array.prototype.some, 'function');
        assert.equal(typeof [].some, 'function');

        assert.equal([101, 199, 250, 20].some(function(element, index, array) {
                                            return element >= 100;
                                        }), true);

        assert.equal([11, 99, 50, 20].some(function(element, index, array) {
                                          return element >= 100;
                                    }), false);
    });
/*
    QUnit.test('fallbacks.Array.unique', function(assert) {
        assert.equal(typeof Array.prototype.unique, 'function');
        assert.equal(typeof [].unique, 'function');

        assert.deepEqual([1, 2, 1, 4, 5, 4].unique(), [1, 2, 4, 5]);
    });
*/
}
...
/*
QUnit.test('fallbacks.String.trim', function(assert) {
    assert.equal('', ''.trim());
    assert.equal('', '   '.trim());
    assert.equal('', '\t\t'.trim());
...
QUnit.test('fallbacks.String.ltrim', function(assert) {
    assert.equal('', ''.ltrim());
    assert.equal('', '   '.ltrim());
    assert.equal('', '\t\t'.ltrim());
    assert.equal('abc', 'abc'.ltrim());
    assert.equal('abc', 'abc'.ltrim());
    assert.equal('abc  ', 'abc  '.ltrim());
    assert.equal('abc  ', '  abc  '.ltrim());
    assert.equal('abc', '\tabc'.ltrim());
...
QUnit.test('fallbacks.String.rtrim', function(assert) {
    assert.equal('', ''.rtrim());
    assert.equal('', '   '.rtrim());
    assert.equal('', '\t\t'.rtrim());
    assert.equal('abc', 'abc'.rtrim());
    assert.equal('  abc', '  abc'.rtrim());
    assert.equal('abc', 'abc  '.rtrim());
    assert.equal('  abc', '  abc  '.rtrim());
    assert.equal('\tabc', '\tabc'.rtrim());
...
QUnit.test('fallbacks.String.startsWith', function(assert) {
    assert.equal(''.startsWith(''), true);
    assert.equal(''.startsWith('a'), false);

    assert.equal('a'.startsWith('a'), true);
    assert.equal('abcd'.startsWith('a'), true);
    assert.equal('abcd'.startsWith('abcd'), true);

    assert.equal('d'.startsWith('a'), false);
    assert.equal('dcba'.startsWith('a'), false);
    assert.equal('dcba'.startsWith('abcd'), false);
});

QUnit.test('fallbacks.String.endsWith', function(assert) {
    assert.equal(''.endsWith(''), true);
    assert.equal(''.endsWith('a'), false);

    assert.equal('d'.endsWith('d'), true);
    assert.equal('abcd'.endsWith('d'), true);
    assert.equal('abcd'.endsWith('abcd'), true);

    assert.equal('a'.endsWith('d'), false);
    assert.equal('dcba'.endsWith('d'), false);
    assert.equal('dcba'.startsWith('abcd'), false);
});
*/
