...
class CremePropertyTypeTestCase(CremeTestCase):
    ...
    def test_manager_proxy__get_or_create__more_arguments(self):
        ...
        self.assertListEqual([contact_ct, orga_ct], [*proxy.subject_ctypes])
    ...
    def test_manager_proxy__update_or_create(self):
        ...
        ptype2 = CremePropertyType.objects.proxy(
            ...
            subject_ctypes=[FakeContact, FakeActivity],
        ).update_or_create()
        self.assertIsInstance(ptype2, CremePropertyType)
    ...
    def test_manager_proxy__update_ctypes(self):
        get_ct = ContentType.objects.get_for_model
        contact_ct = get_ct(FakeContact)
        orga_ct = get_ct(FakeOrganisation)
        ...
        proxy = CremePropertyType.objects.proxy(
            uuid=uuid4(), text='Is smart', subject_ctypes=[FakeContact],
        )

        proxy.add_ctypes(FakeOrganisation, get_ct(FakeActivity), FakeContact)
        self.assertCountEqual(
            [contact_ct, orga_ct, activity_ct], [*proxy.subject_ctypes],
        )

        proxy.remove_ctypes(FakeOrganisation, contact_ct)
