...
class EntityFilterManagerTestCase(CremeTestCase):
    ...
    def test_smart_update_or_create__admin_owner(self):
        "'admin' owner."
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_smart_update_or_create__private_sub_filters(self):
        """Private sub-filters:
    ...
    def test_smart_update_or_create__invalid_pk(self):
        "Invalid chars in PK."
    ...
    def test_smart_update_or_create__again__custom(self):
    ...
    def test_smart_update_or_create__again__not_custom(self):
        ...
        name = 'Misato my love'
        efilter = create_efilter(pk, name, FakeContact, user=user, conditions=conditions)
        self.assertEqual(name, efilter.name)
        ...
        with self.assertRaises(ValueError):
            create_efilter(
                pk, name, FakeContact,
                user=user, conditions=conditions, is_custom=True,
            )

    def test_smart_update_or_create__again__content_type_change(self):
    ...
    def test_smart_update_or_create__again__content_type_change__custom(self):
    ...
    def test_smart_update_or_create__again__new_version(self):
...
class EntityFilterConditionTestCase(CremeTestCase):
    ...
    def test_equal__dict_comparison(self):
...
class EntityFilterTestCase(CremeTestCase):
    ...
    def test_can_edit__root(self):  # DEPRECATED
        ...
        # KO = (
        #     False, _('You are not allowed to edit this filter (you are not the owner)'),
        # )
        self.assertTupleEqual(
            # KO,
            (False, _('You are not the owner of this private filter')),
            EntityFilter(entity_type=FakeContact, user=other, is_private=True).can_edit(root),
        )
        self.assertTupleEqual(
        ...
        self.assertTupleEqual(
            # KO,
            (False, _('You do not belong to the owner-team of this private filter')),
            EntityFilter(entity_type=FakeContact, user=team2, is_private=True).can_edit(root),
        )
        self.assertTupleEqual(
    ...
    def test_can_edit__regular_user(self):  # DEPRECATED
        ...
        self.assertTupleEqual(
            (False, _('Only superusers can edit this filter (no owner)')),
    ...
    def test_can_edit__regular_user__setting_value_true(self):  # DEPRECATED
        ...
        OK = (True, 'OK')
        self.assertTupleEqual(
            OK, EntityFilter(user=user, entity_type=FakeContact).can_edit(user),
        )
        self.assertTupleEqual(
            OK, EntityFilter(entity_type=FakeContact).can_edit(user),
        )

    def test_can_delete__root(self):  # DEPRECATED
        root = self.get_root_user()
        other = self.create_user()
        team1 = self.create_team('team A', other, root)
        team2 = self.create_team('team B', other)
    ...
    def test_can_delete__staff(self):  # DEPRECATED
        staff = self.create_user(is_staff=True)
        self.assertTrue(
            EntityFilter(
                entity_type=FakeContact, user=self.get_root_user(), is_private=True,
    ...
    def test_can_delete__regular_user(self):  # DEPRECATED
        from creme.documents import get_document_model
        ...
        self.assertTupleEqual(
            (False, _('Only superusers can delete this filter (no owner)')),
    ...
    def test_can_delete__regular_user__setting_value_true(self):  # DEPRECATED
        setting_value = SettingValue.objects.get_4_key(global_filters_edition_key)
        setting_value.value = True
        setting_value.save()
    ...
    def test_can_view__root(self):  # DEPRECATED
        root = self.get_root_user()
        other = self.create_user()
        team1 = self.create_team('team A', other, root)
        team2 = self.create_team('team B', other)
    ...
    def test_can_view__staff(self):  # DEPRECATED
        staff = self.create_user(is_staff=True)
        self.assertTrue(
            EntityFilter(
                entity_type=FakeContact, user=self.get_root_user(), is_private=True,
    ...
    def test_can_view__regular_user(self):  # DEPRECATED
        from creme.documents import get_document_model
    ...
    def test_check_edition__root(self):
        ...
        with self.assertNoException():
            EntityFilter(entity_type=FakeContact).check_edition(root)

        with self.assertNoException():
            EntityFilter(entity_type=FakeContact, user=other).check_edition(user=root)
        ...
        self.assertEqual(
            _('You are not the owner of this private filter'), str(cm.exception),
        )

        with self.assertNoException():
            EntityFilter(entity_type=FakeContact, user=root, is_private=True).check_edition(root)

        with self.assertRaises(PermissionDenied) as cm:
            EntityFilter(entity_type=FakeContact, user=team2, is_private=True).check_edition(root)
        self.assertEqual(
            _('You do not belong to the owner-team of this private filter'),
            str(cm.exception),
        )

        with self.assertNoException():
            EntityFilter(entity_type=FakeContact, user=team1, is_private=True).check_edition(root)
    ...
    def test_check_edition__regular_user(self):
        ...
        user = self.create_user(role=self.create_role(allowed_apps=['creme_core']))
        with self.assertNoException():
            EntityFilter(user=user, entity_type=FakeContact).check_edition(user)

        with self.assertRaises(PermissionDenied) as cm:
            EntityFilter(entity_type=FakeContact).check_edition(user)
        self.assertEqual(
        ...
        with self.assertRaises(PermissionDenied) as cm:
            EntityFilter(entity_type=get_document_model()).check_edition(user)
        self.assertEqual(
            _('You are not allowed to access to this app'),
            str(cm.exception),
        )
    ...
    def test_check_deletion__root(self):
        root = self.get_root_user()
        other = self.create_user()
        team1 = self.create_team('team A', other, root)
        team2 = self.create_team('team B', other)
        ...
        with self.assertNoException():
            EntityFilter(entity_type=FakeContact).check_deletion(user=root)

        with self.assertNoException():
            EntityFilter(entity_type=FakeContact, user=other).check_deletion(root)
        ...
        with self.assertRaises(ConflictError) as cm:
            EntityFilter(entity_type=FakeContact, is_custom=False).check_deletion(root)
        self.assertEqual(
            _("This filter can't be deleted (system filter)"), str(cm.exception),
        )

    def test_check_deletion__staff(self):
        staff = self.create_user(is_staff=True)
        with self.assertNoException():
            EntityFilter(
                entity_type=FakeContact, user=self.get_root_user(), is_private=True,
            ).check_deletion(staff)

    def test_check_deletion__regular_user(self):
        from creme.documents import get_document_model
        ...
        user = self.create_user(role=self.create_role(allowed_apps=['creme_core']))
        with self.assertNoException():
            EntityFilter(user=user, entity_type=FakeContact).check_deletion(user)

        with self.assertRaises(PermissionDenied) as cm:
            EntityFilter(entity_type=FakeContact).check_deletion(user)
        self.assertEqual(
        ...
        with self.assertRaises(PermissionDenied) as cm:
            EntityFilter(entity_type=get_document_model()).check_deletion(user)
        self.assertEqual(
            _('You are not allowed to access to this app'), str(cm.exception),
        )

    def test_check_deletion__regular_user__setting_value_true(self):
        setting_value = SettingValue.objects.get_4_key(global_filters_edition_key)
        setting_value.value = True
        setting_value.save()
    ...
    def test_check_view__root(self):
        root = self.get_root_user()
        other = self.create_user()
        team1 = self.create_team('team A', other, root)
        team2 = self.create_team('team B', other)
        ...
        with self.assertNoException():
            EntityFilter(entity_type=FakeContact).check_view(user=root)

        with self.assertNoException():
            EntityFilter(entity_type=FakeContact, user=other).check_view(root)

        with self.assertRaises(PermissionDenied) as cm:
            EntityFilter(entity_type=FakeContact, user=other, is_private=True).check_view(root)
        self.assertEqual(
            _('You are not the owner of this private filter'), str(cm.exception),
        )

        with self.assertNoException():
            EntityFilter(entity_type=FakeContact, user=root, is_private=True).check_view(root)

        with self.assertRaises(PermissionDenied) as cm:
            EntityFilter(entity_type=FakeContact, user=team2, is_private=True).check_view(root)
        self.assertEqual(
            _('You do not belong to the owner-team of this private filter'),
            str(cm.exception),
        )
        ...
        with self.assertNoException():
            EntityFilter(entity_type=FakeContact, user=team1, is_private=True).check_view(root)

    def test_check_view__staff(self):
        staff = self.create_user(is_staff=True)
        with self.assertNoException():
            EntityFilter(
                entity_type=FakeContact, user=self.get_root_user(), is_private=True,
    ...
    def test_check_view__regular_user(self):
        from creme.documents import get_document_model

        user = self.create_user(role=self.create_role(allowed_apps=['creme_core']))
        with self.assertNoException():
            EntityFilter(user=user, entity_type=FakeContact).check_view(user)

        with self.assertNoException():
            EntityFilter(entity_type=FakeContact).check_view(user)

        with self.assertRaises(PermissionDenied) as cm:
            EntityFilter(entity_type=get_document_model()).check_view(user)
        self.assertEqual(
            _('You are not allowed to access to this app'),
            str(cm.exception),
        )
    ...
    def test_set_conditions__one_conditions(self):
        build = partial(RegularFieldConditionHandler.build_condition, model=FakeContact)
        efilter = EntityFilter.objects.smart_update_or_create(
        ...
        # NB: create another condition that has the last id (so if we delete the
    ...
    def test_set_conditions__several_conditions(self):
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', 'Jet', FakeContact, is_custom=True,
        )
    ...
    def test_set_conditions__erroneous_conditions(self):
    ...
    def test_get_verbose_conditions__no_condition(self):
        ...
        self.assertEqual([], [*efilter.get_verbose_conditions(self.get_root_user())])

    def test_get_verbose_conditions__one_condition(self):
        first_name = 'Misato'
        efilter = EntityFilter.objects.smart_update_or_create(
        ...
        user = self.get_root_user()
        self.assertEqual(msg, efilter.get_conditions()[0].description(user))
        self.assertListEqual([msg], [*efilter.get_verbose_conditions(user)])

    def test_get_verbose_conditions__several_conditions(self):
        name = 'Nerv'
        desc1 = 'important'
        desc2 = 'beware'
        efilter = EntityFilter.objects.smart_update_or_create(
        ...
        self.assertListEqual(
            ...
            [*efilter.get_verbose_conditions(self.get_root_user())],
        )


class EntityFilterFilteringTestCase(CremeTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def setUp(self):
        super().setUp()
        self.user = user = self.get_root_user()
    ...
    def test_regular_field__equals(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__equals__several_values(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__equals__boolean(self):
        "Boolean field."
        contacts = self.contacts
    ...
    def test_regular_field__equals__current_user(self):
        other_user = self.create_user(1)
    ...
    def test_regular_field__equals__current_user__teams(self):
        user = self.user
        other_user = self.create_user(1)
        teammate = self.create_user(2)
    ...
    def test_regular_field__iequals(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__not_equals(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__not_equals__current_user(self):
        user = self.user
        other_user = self.create_user(1)
        teammate = self.create_user(2)
    ...
    def test_regular_field__not_iequals(self):
        pk = 'test-filter01'
    ...
    def test_regular_field__contains(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__icontains(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__contains_not(self):
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', 'Not Ikari', FakeContact, is_custom=True,
        ).set_conditions([
            RegularFieldConditionHandler.build_condition(
    ...
    def test_regular_field__icontains_not(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__gt(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__gte(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__lt(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__lte(self):
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', '<= Faye', FakeContact, is_custom=True,
            conditions=[
                RegularFieldConditionHandler.build_condition(
    ...
    def test_regular_field__startswith(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__istartswith(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__startswith_not(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__istartswith_not(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__endswith(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__iendswith(self):
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', 'ends "SATO"', FakeContact, is_custom=True,
            conditions=[
                RegularFieldConditionHandler.build_condition(
    ...
    def test_regular_field__endswith_not(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__iendswith_not(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__isempty__true(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__isempty__false(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__isempty__not_charfield(self):
    ...
    def test_regular_field__isempty__subfield(self):
        "Subfield of FK."
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_regular_field__range(self):
        create_orga = partial(FakeOrganisation.objects.create, user=self.user)
    ...
    def test_filter__fk__subfield(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_accept__one_condition(self):
        user = self.get_root_user()
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', 'Ikari', FakeContact,
            conditions=[
    ...
    def test_accept__two_conditions__and(self):
        user = self.get_root_user()
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', 'Ikari', FakeContact,
            conditions=[
    ...
    def test_accept__two_conditions__or(self):
        user = self.get_root_user()
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', 'Ikari', FakeContact,
            use_or=True,
    ...
    def test_multi_conditions__and(self):
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_multi_conditions__or(self):
        build = partial(RegularFieldConditionHandler.build_condition, model=FakeContact)
        efilter = EntityFilter.objects.smart_update_or_create(
    ...
    def test_subfilter__cycle_error__length_is_0(self):
    ...
    def test_subfilter__cycle_error__length_is_1(self):
    ...
    def test_subfilter__cycle_error__length_is_2(self):
    ...
    def test_properties__sevreal_conditions(self):
    ...
    def test_property_type_deletion(self):
    ...
    def test_relations__just_rtype(self):
        "No CT/entity."
    ...
    def test_relations__fixed_ctype(self):
        loves = self._aux_test_relations()
    ...
    def test_relations__fixed_entity(self):
        loves = self._aux_test_relations()
    ...
    def test_relations__fixed_entity__deleted(self):
    ...
    def test_relations__several_conditions__or(self):
    ...
    def test_relations__several_conditions__and(self):
    ...
    def test_relations_subfilter(self):
        loves = self._aux_test_relations()
        in_love = ('shinji', 'gendou')
    ...
    def test_relations_subfilter__cycle_error__length_is_0(self):
        loves = self._aux_test_relations()
    ...
    def test_relations_subfilter__cycle_error__length_is_1(self):
        loves = self._aux_test_relations()
    ...
    def test_relations_subfilter__deleted_rtype(self):
        loves = RelationType.objects.builder(
            id='test-subject_love', predicate='Is loving',
        ).symmetric(id='test-object_love', predicate='Is loved by').get_or_create()[0]
        hates = RelationType.objects.builder(
    ...
    def test_relations_subfilter__several_conditions__or(self):
        loves = self._aux_test_relations()
    ...
    def test_relations_subfilter__several_conditions__and(self):
        loves = self._aux_test_relations()
    ...
    def test_date__relative_to_now(self):
    ...
    def test_datetime__previous_year(self):
        faye = self.contacts['faye']
    ...
    def test_datetime__current_year(self):
        faye = self.contacts['faye']
        FakeContact.objects.filter(pk=faye.id).update(
            created=faye.created - timedelta(days=faye.created.month * 31),
        )
    ...
    def test_datetime__next_year(self):
        faye = self.contacts['faye']
        FakeContact.objects.filter(pk=faye.id).update(
    ...
    def test_datetime__current_month(self):
        faye = self.contacts['faye']
    ...
    def test_datetime__current_quarter(self):
        faye = self.contacts['faye']
        FakeContact.objects.filter(pk=faye.id).update(
    ...
    def test_datetime__subfield(self):
        contacts = self.contacts
        spike = contacts['spike']
        jet   = contacts['jet']
    ...
    def test_custom_field__one_int(self):
    ...
    def test_custom_field__two_int(self):
    ...
    def test_custom_field__str__contains_not(self):
    ...
    def test_custom_field__2_conditions(self):
    ...
    def test_custom_field__decimal__isempty(self):
        rei = self.contacts['rei']
    ...
    def test_custom_field__boolean__equals__true(self):
        contacts = self.contacts
        ed  = contacts['ed']
        rei = contacts['rei']
    ...
    def test_custom_field__boolean__equals__false(self):
        contacts = self.contacts
        ed  = contacts['ed']
        rei = contacts['rei']
    ...
    def test_custom_field__boolean__equals_not(self):
        rei = self.contacts['rei']
    ...
    def test_custom_field__enum__several_values(self):
        c = self.contacts
        rei    = c['rei']
        asuka  = c['asuka']
        shinji = c['shinji']
    ...
    def test_custom_field__deletion(self):
        create_cf = partial(
            CustomField.objects.create,
            content_type=self.contact_ct, field_type=CustomField.INT,
        )
    ...
    def test_custom_field__deletion__date(self):
        "Date custom field."
        create_cf = partial(
            CustomField.objects.create,
    ...
    def test_custom_field_enum_isempty(self):
        contacts = self.contacts
        rei = contacts['rei']
    ...
    def test_custom_field_string_isempty(self):
        c = self.contacts
    ...
    def test_custom_field_boolean_isempty(self):
        custom_field = CustomField.objects.create(
    ...
    def test_date_custom_field__gte(self):
    ...
    def test_date_custom_field__lte(self):
        custom_field = self._aux_test_datecf()
    ...
    def test_date_custom_field__range(self):
        custom_field = self._aux_test_datecf()
    ...
    def test_date_custom_field__relative_to_now(self):
        custom_field = CustomField.objects.create(
    ...
    def test_date_custom_field__conditions_on_2_fields(self):
    ...
    def test_invalid__regular_field(self):
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', 'Ikari', FakeContact, is_custom=True,
        )
        build = partial(
    ...
    def test_invalid__date_field(self):
        efilter = EntityFilter.objects.smart_update_or_create(
            'test-filter01', 'Ikari', FakeContact, is_custom=True,
        )
        cond1 = RegularFieldConditionHandler.build_condition(
    ...
    def test_invalid__subfilter(self):
    ...
    def test_invalid__relations_subfilter(self):
        loves = self._aux_test_relations()
...
class EntityFilterListTestCase(CremeTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.contact_ct = ContentType.objects.get_for_model(FakeContact)
    ...
    def test_several_owners(self):
        ...
        ef1 = create_ef(1)
        ef2 = create_ef(2,   user=super_user)
        ef3 = create_ef(3,   user=other_user)
        ef4 = create_ef(4,   user=tt_team)
        ef5 = create_ef(5,   user=a_team)
        ef6 = create_ef(6,   user=super_user, is_private=True, is_custom=True)
        ef7 = create_ef(7,   user=tt_team,    is_private=True, is_custom=True)
        ef8 = create_ef(8,   user=other_user, is_private=True, is_custom=True)
        ef9 = create_ef(9,   user=a_team,     is_private=True, is_custom=True)
        ef10 = create_ef(10, user=teammate,   is_private=True, is_custom=True)

        # self.assertEqual(ef1.can_view(super_user), (True, 'OK'))
        with self.assertNoException():
            ef1.check_view(super_user)
        # self.assertIs(ef8.can_view(super_user)[0], False)
        self.assertRaises(PermissionDenied, ef8.check_view, super_user)
        ...
        self.assertIn(ef4, efl)
        self.assertIn(ef5, efl)
        self.assertIn(ef6, efl)
        self.assertIn(ef7, efl)
        self.assertNotIn(ef8, efl)
        self.assertNotIn(ef9, efl)
