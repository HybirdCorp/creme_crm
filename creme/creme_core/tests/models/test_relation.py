...
class RelationTypeManagerTestCase(CremeTestCase):
    ...
    def test_proxy__update_or_create(self):
        subject_id = 'test-subject_foobar'
        subject_pred = 'is loving'
        object_id = 'test-object_foobar'
        object_pred  = 'is loved by'

        proxy = RelationType.objects.proxy(
            id=subject_id, predicate=subject_pred,
        ).symmetric(id=object_id, predicate=object_pred)

        self.assertEqual(proxy.id,        subject_id)
        self.assertEqual(proxy.predicate, subject_pred)
        self.assertFalse(proxy.is_internal)
        self.assertFalse(proxy.is_custom)
        self.assertTrue(proxy.is_copiable)
        self.assertFalse(proxy.minimal_display)
        self.assertTrue(proxy.enabled)
        self.assertFalse([*proxy.subject_models])
        self.assertFalse([*proxy.subject_ctypes])
        # self.assertFalse([], [*proxy.object_models]) TODO?
        self.assertFalse([*proxy.subject_properties])
        # self.assertFalse([], [*proxy.object_properties]) TODO?
        self.assertFalse([*proxy.subject_forbidden_properties])

        sym_proxy = proxy.symmetric_type
        self.assertEqual(sym_proxy.id,        object_id)
        self.assertEqual(sym_proxy.predicate, object_pred)
        self.assertFalse(sym_proxy.is_internal)
        self.assertFalse(sym_proxy.is_custom)
        self.assertTrue(sym_proxy.is_copiable)
        self.assertFalse(sym_proxy.minimal_display)
        self.assertTrue(sym_proxy.enabled)

        self.assertIs(proxy, sym_proxy.symmetric_type)

        rtype1 = proxy.update_or_create()
        self.assertIsInstance(rtype1, RelationType)
    ...
    def test_proxy__update_or_create__constraints(self):
        ...
        self.assertCountEqual(
            [FakeContact, FakeOrganisation],
            [ct.model_class() for ct in proxy.subject_ctypes],
        )
        self.assertCountEqual([ptype1, ptype2], [*proxy.subject_properties])
        self.assertCountEqual([ptype4, ptype5], [*proxy.subject_forbidden_properties])

        self.assertTrue(proxy.symmetric_type.is_custom)
    ...
    def test_proxy__disabled(self):
        ...
        sym_proxy = proxy.symmetric_type
        self.assertFalse(sym_proxy.enabled)
        ...
        proxy.enabled = True
        self.assertTrue(proxy.enabled)
        self.assertTrue(sym_proxy.enabled)  # Synchronised

        # ---
        incomplete  = RelationType.objects.proxy(
    ...
    def test_proxy__is_custom(self):
        ...
        self.assertTrue(proxy.is_custom)
        self.assertTrue(sym_proxy.is_custom)  # Synchronised
    ...
    def test_proxy__is_internal(self):
        proxy = RelationType.objects.proxy(
            id='test-subject_testproxy', predicate='knows',
            is_internal=True,  # <==
        ).symmetric(id='test-object_testproxy', predicate='is known by')
        self.assertTrue(proxy.is_internal)

        sym_proxy = proxy.symmetric_type
        self.assertTrue(sym_proxy.is_internal)

        rtype1 = proxy.update_or_create()
        self.assertTrue(rtype1.is_internal)
        self.assertTrue(rtype1.symmetric_type.is_internal)

        proxy.is_internal = False
        self.assertFalse(proxy.is_internal)
        self.assertFalse(sym_proxy.is_internal)  # Synchronised
    ...
    def test_proxy__is_copiable(self):
        proxy1 = RelationType.objects.proxy(
        ...
        rtype2 = proxy2.update_or_create()
        self.assertTrue(rtype2.is_copiable)
        self.assertFalse(rtype2.symmetric_type.is_copiable)

    def test_proxy__minimal_display(self):
        proxy1 = RelationType.objects.proxy(
            id='test-subject_minimal_display1', predicate='knows',
            minimal_display=True,  # <==
        ).symmetric(id='test-object_minimal_display1', predicate='is known by')
    ...
    def test_proxy__symmetric_errors(self):
        proxy = RelationType.objects.proxy(
            id='test-subject_err', predicate='knows',
        )

        with self.assertRaises(RuntimeError):
            proxy.symmetric_type  # NOQA

        # update_or_create() ---
        with self.assertRaises(RuntimeError):
            proxy.update_or_create()

        # Call twice ---
        proxy.symmetric(id='test-object_err', predicate='whatever')
        with self.assertRaises(RuntimeError):
            proxy.symmetric(id='test-object_err', predicate='is known by')

    def test_proxy__edit_subject_models(self):
        proxy = RelationType.objects.proxy(
        ...
        proxy.add_subject_models(FakeOrganisation, FakeDocument)
        self.assertCountEqual(
            [FakeContact, FakeOrganisation, FakeDocument],
            [*proxy.subject_models],
        )

        proxy.remove_subject_models(FakeOrganisation, FakeContact)
        self.assertListEqual([FakeDocument], [*proxy.subject_models])

        sym = proxy.symmetric_type
        sym.add_subject_models(FakeContact)
        self.assertListEqual([FakeContact], [*sym.subject_models])

    def test_proxy__edit_properties(self):
        create_ptype = CremePropertyType.objects.create
        ptype1 = create_ptype(text='Test #1')
        ptype2 = create_ptype(text='Test #2')
        ptype3 = create_ptype(text='Test #3')
        ...
        proxy.add_subject_properties(str(ptype2.uuid), str(ptype3.uuid))
        self.assertCountEqual(
            [ptype1, ptype2, ptype3], [*proxy.subject_properties],
        )

        proxy.remove_subject_properties(str(ptype2.uuid), str(ptype1.uuid))
        self.assertListEqual([ptype3], [*proxy.subject_properties])

        sym = proxy.symmetric_type
        sym.add_subject_properties(str(ptype2.uuid))
        self.assertListEqual([ptype2], [*sym.subject_properties])

    def test_proxy__edit_forbidden_properties(self):
        create_ptype = CremePropertyType.objects.create
        ptype1 = create_ptype(text='Test #1')
        ptype2 = create_ptype(text='Test #2')
        ptype3 = create_ptype(text='Test #3')
        ...
        proxy.add_subject_forbidden_properties(str(ptype2.uuid), str(ptype3.uuid))
        self.assertCountEqual(
            [ptype1, ptype2, ptype3], [*proxy.subject_forbidden_properties],
        )

        proxy.remove_subject_forbidden_properties(str(ptype2.uuid), str(ptype1.uuid))
        self.assertListEqual([ptype3], [*proxy.subject_forbidden_properties])
