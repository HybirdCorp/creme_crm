...
class CustomEntityTestCase(BrickTestCaseMixin, CremeTestCase):
    ...
    def test_detail(self):
        ...
        type_name = 'Training'
        self._enable_type(id=1, name=type_name)

        entity = CustomEntity1.objects.create(user=user, name='Training 01')

        url = entity.get_absolute_url()
        self.assertEqual(
            reverse('creme_core__view_custom_entity', args=(1, entity.id,)),
            url,
        )
    ...
    def test_detail__other_type(self):
        user = self.login_as_root_and_get()
        type_name = 'Store'
        self._enable_type(id=2, name=type_name)

        entity = CustomEntity2.objects.create(user=user, name='Store 01')

        url = entity.get_absolute_url()
        self.assertEqual(
            reverse('creme_core__view_custom_entity', args=(2, entity.id,)),
            url,
        )
    ...
    def test_detail__disabled_type(self):
        user = self.login_as_root_and_get()
        # self._enable_type(...)  # NOPE

        entity = CustomEntity1.objects.create(user=user, name='Store 01')
    ...
    def test_creation(self):
        ...
        self.assertEqual(
            _('Create a «{custom_model}»').format(custom_model=ce_type.name),
