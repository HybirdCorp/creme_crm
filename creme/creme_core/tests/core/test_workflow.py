...
class WorkflowTestCase(CremeTestCase):
    ...
    def test_registry__source_formfields(self):
        user = self.get_root_user()
        registry = WorkflowRegistry()
    ...
    def test_registry__source_formfields__extended1(self):
        user = self.get_root_user()
        registry = WorkflowRegistry().register_action_sources(
            FixedEntitySource,
            CreatedEntitySource,
        ...
        self.assertEqual('edited_entity|fixed_entity', kind_id2)
        self.assertIsInstance(ffield2, FixedEntitySourceField)

    def test_registry__source_formfields__extended2(self):
        user = self.get_root_user()
        registry = WorkflowRegistry().register_action_sources(
            CreatedEntitySource,
            EditedEntitySource,
        ...
        self.assertIsList(ffields, length=6)
        ...
        self.assertEqual('subject_entity|entity_fk', kind_id3)
        self.assertIsInstance(ffield3, EntityFKSourceField)
        self.assertEqual(SubjectEntitySource(model=FakeOrganisation), ffield3.entity_source)

        kind_id4, ffield4 = ffields[3]
        self.assertEqual('subject_entity|first_related', kind_id4)
        self.assertIsInstance(ffield4, FirstRelatedEntitySourceField)
        self.assertEqual(SubjectEntitySource(model=FakeOrganisation), ffield4.subject_source)

        kind_id5, ffield5 = ffields[4]
        self.assertEqual('object_entity|entity_fk', kind_id5)
        self.assertIsInstance(ffield5, EntityFKSourceField)
        self.assertEqual(ObjectEntitySource(model=FakeContact), ffield5.entity_source)
