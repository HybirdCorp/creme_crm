...
class WorkflowTestCase(CremeTestCase):
    ...
    def test_registry__build_action_source(self):
        ...
        self.assertEqual(
            FromContextSource(context_key),
            registry.build_action_source(
        ...
        source2 = registry.build_action_source(
            ...
            FixedEntitySource(entity=entity).to_dict()
        )
        ...
        self.assertEqual(FromContextSource(context_key2), source3.instance_source)
    ...
    def test_registry__build_action(self):
        ...
        action2 = registry.build_action(
            RelationAddingAction(
                ...
                rtype=rtype.id,
                object_source=FromContextSource(ctxt_key2),
            ).to_dict()
        )
        self.assertIsInstance(action2, RelationAddingAction)
        self.assertEqual(rtype, action2.relation_type)
        self.assertEqual(FromContextSource(ctxt_key1), action2.subject_source)
        self.assertEqual(FromContextSource(ctxt_key2), action2.object_source)
