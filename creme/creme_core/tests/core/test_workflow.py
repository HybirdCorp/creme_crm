...
class WorkflowEventsTestCase(CremeTestCase):
    ...
    def test_property_added(self):
        ...
        prop = CremeProperty.objects.create(
            creme_entity=entity, type=ptype,
        )

        evt = PropertyAdded(creme_property=prop)
        self.assertIsInstance(evt, WorkflowEvent)
        self.assertEqual(prop, evt.creme_property)
...
class WorkflowRegistryTestCase(CremeTestCase):
    ...
    def test_broken_action_source(self):
        message = 'Model is invalid'
        source = BrokenActionSource(message=message)
    ...
    def test_register_action_sources(self):
        registry = WorkflowRegistry()
    ...
    def test_register_action_sources__empty_id(self):
        ...
        with self.assertRaises(registry.RegistrationError):
            registry.register_action_sources(EmptyIDSource)

    def test_register_action_sources__duplicated_id(self):
        class DuplicatedIDSource(CreatedEntitySource):
            pass

        registry = WorkflowRegistry().register_action_sources(CreatedEntitySource)

        with self.assertRaises(registry.RegistrationError):
            registry.register_action_sources(DuplicatedIDSource)

    def test_register_action_sources__invalid_char_in_id(self):
        ...
        with self.assertRaises(registry.RegistrationError):
            registry.register_action_sources(InvalidIDSource)

    def test_unregister_action_sources__error(self):
        registry = WorkflowRegistry()
    ...
    def test_build_action_source(self):
        ...
        self.assertEqual(EditedEntitySource(model=FakeOrganisation), source3.entity_source)

    def test_build_action_source__invalid_id(self):
        type_id = 'invalid'
        source = WorkflowRegistry().build_action_source({'type': type_id})
        self.assertIsInstance(source, BrokenActionSource)
        self.assertEqual(
            _(
    ...
    def test_build_action_source__fatal(self):
        msg = 'The key "foobar" was not found.'

        class ExplodingSource(WorkflowActionSource):
            type_id = 'exploding'
            verbose_name = 'Explosion!!'
        ...
        registry = WorkflowRegistry().register_action_sources(ExplodingSource)
        action = registry.build_action_source({'type': ExplodingSource.type_id})
        self.assertIsInstance(action, BrokenActionSource)
        self.assertEqual(
            _(
    ...
    def test_source_formfields(self):
        ...
        self.assertListEqual(
            [], registry.action_source_formfields(root_sources=[], user=user)
        )

        # ---
        ffields = registry.action_source_formfields(
    ...
    def test_global(self):
        ...
        self.assertIn(TaggedEntitySource,       action_sources)
        ...
        self.assertIn(EntityFKSource,           action_sources)
