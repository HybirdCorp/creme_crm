...
class RegularFieldConditionHandlerTestCase(_ConditionHandlerTestCase):
    ...
    def test_accept__fk(self):
        ...
        # Deleted instance ---
        del_handler = RegularFieldConditionHandler(
            efilter_type=EF_REGULAR,
            model=FakeOrganisation,
            field_name='sector',
            operator_id=operators.EQUALS,
        ...
        with self.assertNoException():
            accepted = del_handler.accept(entity=o1, user=user)
    ...
    def test_accept__m2m(self):
        ...
        # EQUALS ---
        eq_handler = RegularFieldConditionHandler(
            efilter_type=EF_REGULAR,
            model=FakeDocument,
        ...
        self.assertIs(eq_handler.accept(entity=doc1, user=user), True)
        self.assertIs(eq_handler.accept(entity=doc2, user=user), False)
        self.assertIs(eq_handler.accept(entity=doc3, user=user), False)

        # ISEMPTY ---
        empty_handler = RegularFieldConditionHandler(
            efilter_type=EF_REGULAR,
            model=FakeDocument,
            field_name='categories',
            operator_id=operators.ISEMPTY,
        ...
        self.assertIs(empty_handler.accept(entity=doc1, user=user), False)
        self.assertIs(empty_handler.accept(entity=doc2, user=user), False)
        self.assertIs(empty_handler.accept(entity=doc3, user=user), True)
        ...
        # ISEMPTY (False) ---
        notempty_handler = RegularFieldConditionHandler(
            efilter_type=EF_REGULAR,
            model=FakeDocument,
            field_name='categories',
            operator_id=operators.ISEMPTY,
        ...
        self.assertIs(notempty_handler.accept(entity=doc1, user=user), True)
        self.assertIs(notempty_handler.accept(entity=doc2, user=user), True)
        self.assertIs(notempty_handler.accept(entity=doc3, user=user), False)
        ...
        # with self.assertLogs(level='CRITICAL'):
        with self.assertNoException():
            accepted = del_handler.accept(entity=doc1, user=user)
        self.assertIs(accepted, False)
...
class CustomFieldConditionHandlerTestCase(_ConditionHandlerTestCase):
    ...
    def test_description__enum(self):
        ...
        handler = CustomFieldConditionHandler(
            ...
            values=[str(enum_A.uuid), str(enum_B.uuid), str(uuid4())],
        )

        with self.assertNumQueries(1):
            description = handler.description(user)
        ...
        # ---
        invalid_handler2 = CustomFieldConditionHandler(
            efilter_type=EF_REGULAR,
            model=FakeContact,
            custom_field=custom_field,
            operator_id=operators.EQUALS,
        ...
        self.assertEqual(
            ...
            invalid_handler2.description(user),
        )

        # ---
        empty_handler = CustomFieldConditionHandler(
            efilter_type=EF_REGULAR,
            model=FakeContact,
            custom_field=custom_field,
            operator_id=operators.ISEMPTY,
        ...
        self.assertEqual(
            ...
            empty_handler.description(user),
        )
    ...
    def test_description__multienum(self):
        ...
        handler = CustomFieldConditionHandler(
            ...
            values=[str(enum_1.uuid), str(enum_2.uuid), str(uuid4())],
        )
