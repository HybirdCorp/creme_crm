...
class RegularFieldsConditionsFieldTestCase(_ConditionsFieldTestCase):
    ...
    def test_clean__invalid_static_choice(self):
    ...
    def test_clean__several_invalid_inputs(self):
        field = RegularFieldsConditionsField(
    ...
    def test_iequals__condition(self):
    ...
    def test_iequals__multiple_as_string(self):
    ...
    def test_iequals__multiple_as_list(self):
        clean = RegularFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        ).clean
        operator = operators.IEQUALS
    ...
    def test_fk(self):
        ...
        # ---
        prepared_value = json_load(field.prepare_value([condition]))[0]
        self.assertDictEqual(
            {
                'name': name,
                'type': 'enum__null',
                'ctype': ContentType.objects.get_for_model(FakeCivility).id,
            },
            prepared_value['field'],
        )
        self.assertEqual(operator, prepared_value['operator']['id'])
        self.assertIn('enum', prepared_value['operator']['types'])  # TODO: complete
        self.assertEqual(str(instance.pk), prepared_value['value'])

    def test_fk__operand(self):
        field = RegularFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        )
        name = 'user'
        operator = operators.EQUALS
        operand_id = '__currentuser__'
        condition = self.get_alone_element(field.clean(self.build_data({
            'operator': operator, 'name': name, 'value': operand_id,
        })))
        self.assertEqual(RegularFieldConditionHandler.type_id, condition.type)
        self.assertEqual(name,                                 condition.name)
        self.assertDictEqual(
            {'operator': operator, 'values': [operand_id]}, condition.value,
        )

        # ---
        prepared_value = json_load(field.prepare_value([condition]))[0]
        self.assertDictEqual({'name': name, 'type': 'user'}, prepared_value['field'])
        self.assertEqual(operator, prepared_value['operator']['id'])
        self.assertIn('enum', prepared_value['operator']['types'])  # TODO: complete
        self.assertEqual(operand_id, prepared_value['value'])
...
class DateFieldsConditionsFieldTestCase(_ConditionsFieldTestCase):
    ...
    def test_ok__start_n_end__filter_type(self):
    ...
    def test_fields_config__sub_fields(self):
        hidden_fname = 'expiration_date'
        FieldsConfig.objects.create(
            content_type=FakeInvoice,
            descriptions=[(hidden_fname, {FieldsConfig.HIDDEN: True})],
...
class CustomFieldsConditionsFieldTestCase(_ConditionsFieldTestCase):
    ...
    def test_clean__missing_value(self):
        self.assertFormfieldError(
            field=CustomFieldsConditionsField(
                model=FakeContact, efilter_type=efilter_registry.id,
            ),
    ...
    def test_clean__integer__model_property(self):
    ...
    def test_clean__integer__invalid_value(self):
        ...
        self.assertDictEqual(
            {
                ...
                # 'values': [value],
                'values': [str(evalue.uuid)],
            },
            condition.value,
        )

    def test_clean__enum__as_string(self):
        clean = CustomFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        ).clean
        operator = operators.EQUALS
        ...
        self.assertDictEqual(
            {
                ...
                'rname': 'customfieldenum',
                'values': [
                    # str(self.cfield_enum_A.pk), str(self.cfield_enum_B.pk)
                    str(self.cfield_enum_A.uuid), str(self.cfield_enum_B.uuid),
                ],
            },
            condition.value,
        )

    def test_clean__enum__as_list(self):
        clean = CustomFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        ).clean
        operator = operators.EQUALS
    ...
    def test_clean__multienum(self):
        clean = CustomFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        ).clean
        operator = operators.EQUALS
        # value = str(self.cfield_multienum_F.pk)
        evalue = self.cfield_multienum_F
        condition = self.get_alone_element(clean(self.build_data({
    ...
    def test_clean__multi_enum__as_string(self):
        clean = CustomFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        ).clean
        operator = operators.EQUALS
        ...
        self.assertDictEqual(
            {
                ...
                'rname': 'customfieldmultienum',
                'values': [
                    # str(self.cfield_multienum_F.pk), str(self.cfield_multienum_H.pk)
                    str(self.cfield_multienum_F.uuid), str(self.cfield_multienum_H.uuid),
                ],
            },
            condition.value,
        )

    def test_clean__multi_enum__as_list(self):
        clean = CustomFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        ).clean
    ...
    def test_clean__empty_string(self):
        clean = CustomFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        ).clean
        operator = operators.EQUALS
    ...
    def test_clean__bad_cfield_types(self):
        field = CustomFieldsConditionsField(
            model=FakeContact, efilter_type=efilter_registry.id,
        )
        msg = _('This custom field is invalid with this model.')
    ...
    def test_deleted__already_used(self):
...
class DateCustomFieldsConditionsFieldTestCase(_ConditionsFieldTestCase):
    ...
    def test_clean__several_errors(self):
        msg = _('Please enter a start date and/or a end date.')
        self.assertFormfieldError(
            field=DateCustomFieldsConditionsField(model=FakeContact),
            value=json_dump([
...
class PropertiesConditionsFieldTestCase(_ConditionsFieldTestCase):
    ...
    def test_disabled_ptype__used(self):
...
class RelationsConditionsFieldTestCase(_ConditionsFieldTestCase):
    ...
    def test_ok__basic(self):
        "No ContentType, no object entity."
    ...
    def test_ok__fixed_ctype_n_entity(self):
        "Fixed ContentType + fixed entity."
        rtype1 = self._create_rtype()
        rtype2 = rtype1.symmetric_type
        user = self.get_root_user()
    ...
    def test_ok__deleted_entity(self):
        "Fixed entity is deleted."
        rtype = self._create_rtype()
        user = self.get_root_user()
...
class EntityFilterFormsTestCase(CremeTestCase):
    ...
    def test_creation_form__other_handlers(self):
        user = self.get_root_user()
    ...
    def test_edition_form__other_handlers(self):
        user = self.get_root_user()
