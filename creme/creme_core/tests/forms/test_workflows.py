...
# from creme.creme_core.workflows import FromContextSource
...
#         self.assertEqual(model1, field1.model)
#
#         exp_source1 = FromContextSource(context_key=EntityCreationTrigger.CREATED)
...
class PropertyAddingActionFieldTestCase(CremeTestCase):
    ...
    def test_ok(self):
        # TODO: use source descriptors to get verbose label for things
        # TODO: source must be selected too
        orga = FakeOrganisation.objects.create(user=self.get_root_user(), name='Acme')
        source = FixedEntitySource(entity=orga)
        field = PropertyAddingActionField(source=source)
        self.assertEqual(source, field.source)

        ptype = CremePropertyType.objects.create(text='Is prosperous')

        # TODO: __eq__ for actions
        # self.assertEqual(
        #     PropertyAddingAction(entity_source=source, ptype=ptype),
        #     field.clean(ptype.id),
        # )
        action = field.clean(ptype.id)
        self.assertIsInstance(action, PropertyAddingAction)
        self.assertEqual(source, action.entity_source)
        self.assertEqual(ptype,  action.property_type)

        self.fail('TODO: sourceSSSS')

    # TODO: test errors
    #   - classical (required, invalid type, invalid data...)
    #   - invalid source
    #   - unknown ptype
    #   - forbidden ptype
