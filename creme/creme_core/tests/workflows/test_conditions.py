...
class WorkflowConditionsTestCase(CremeTestCase):
    def test_one_condition(self):
        ...
        builder = WorkflowConditions.build(
            source=source,
            conditions=[
        ...
        self.assertIsInstance(builder, WorkflowConditions._Builder)

        conditions = builder.do()
        ...
        ctxt_key = CreatedEntitySource.type_id
        self.assertTrue(conditions.accept(
            user=user, context={ctxt_key: create_orga(name=value)},
        ))
        self.assertFalse(conditions.accept(
            user=user, context={ctxt_key: create_orga(name=f'Not {value}')},
        ))
    ...
    def test_zero_condition(self):
        ...
        conditions = WorkflowConditions.build(source=source, conditions=[]).do()
        self.assertIsInstance(conditions, WorkflowConditions)
        ...
        self.assertTrue(conditions.accept(
            user=user, context={EditedEntitySource.type_id: orga},
        ))
    ...
    def test_two_conditions(self):
        ...
        self.assertTrue(conditions.accept(
            user=user,
            context={ctxt_key: create_orga(name=f'{value1} 2000', creation_date=today())},
        ))
        self.assertFalse(conditions.accept(
            user=user,
            context={ctxt_key: create_orga(name=f'Not {value1}', creation_date=today())},
        ))
        self.assertFalse(conditions.accept(
            user=user,
            context={
                ctxt_key: create_orga(
    ...
    def test_two_sources(self):
        ...
        exp_description2 = desc_fmt(
            label=_('Conditions on «{source}»:').format(
                source=source2.render(user=user, mode=source2.HTML),
            ),
        ...
        def build_ctxt():
            return {
                SubjectEntitySource.type_id: self.refresh(orga),
                ObjectEntitySource.type_id: self.refresh(contact),
            }

        # 2 conditions are false
        cfield1.value_class(custom_field=cfield1, entity=orga).set_value_n_save(2)
        self.assertFalse(conditions.accept(user=user, context=build_ctxt()))
        ...
        self.assertTrue(conditions.accept(user=user, context=build_ctxt()))
