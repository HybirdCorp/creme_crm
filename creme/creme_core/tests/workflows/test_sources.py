...
class FirstRelatedEntitySourceTestCase(CremeTestCase):
    ...
    def test_broken_subject_source(self):
        ...
        # self.assertIsInstance(source.subject_source, BrokenActionSource)
        self.assertIsInstance(source.sub_source, BrokenActionSource)
        self.assertIsNone(source.extract({}))

        with self.assertNoException():
            source.render(user=self.get_root_user(), mode=source.TEXT_PLAIN)
    ...
    def test_broken_rtype(self):
        ...
        user = self.get_root_user()
        error_msg = _('The relation type does not exist anymore')
        self.assertEqual(
