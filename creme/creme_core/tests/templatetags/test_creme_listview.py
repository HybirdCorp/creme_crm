...
class CremeListViewTagsTestCase(CremeTestCase):
    def test_listview_pager_slow(self):
        ...
        self.assertInHTML('<a class="pager-link is-disabled pager-link-previous" href="" title="" >{label}</a>'.format(
                              label=_('Previous page')
                          ), rendered)

        self.assertInHTML('<span class="pager-link is-disabled pager-link-current">1</span>',
                          rendered)

        self.assertInHTML('<a class="pager-link pager-link-next" href="" title="{help}" data-page="2">{label}</a>'.format(
    ...
    def test_listview_pager_fast(self):
        ...
        self.assertInHTML('<a class="pager-link is-disabled pager-link-first" href="" title="{label}" >{label}</a>'.format(
                              label=_('First page')
                          ), rendered)

        self.assertInHTML('<a class="pager-link is-disabled pager-link-previous" href="" title="{label}" >{label}</a>'.format(
                              label=_('Previous page')
                          ), rendered)
