...
class CremeBricksTagsTestCase(BrickTestCaseMixin, CremeTestCase):
    ...
    def test_brick_import_n_display__named_brick__invalid(self):
        with self.assertRaises(LookupError) as cm:
            Template(
                "{% load creme_bricks %}"
                "{% brick_import app='creme_core' name='invalid' tag='DETAIL' as my_brick %}"
            ).render(RequestContext(self.build_request()))

        self.assertEqual(
            'Cannot import the brick class with id="regular-creme_core-invalid"',
            str(cm.exception),
        )
