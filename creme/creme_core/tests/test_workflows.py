...
class WorkflowsTestCase(CremeTestCase):
    ...
    def test_trigger__relation_adding__eq(self):
    ...
    def test_source__created_entity__eq(self):
    ...
    def test_source__edited_entity__eq(self):
    ...
    def test_source__subject_entity(self):
    ...
    def test_source__object_entity(self):
    ...
    def test_source__fixed_entity__render(self):
        user = self.get_root_user()
        orga = FakeOrganisation.objects.create(user=user, name='Acme')
    ...
    def test_source__entity_fk(self):
        user = self.get_root_user()
        ...
        source = EntityFKField(entity_source=fixed_source, field_name=field_name)
        self.assertIsInstance(source, WorkflowActionSource)
        ...
        deserialized = EntityFKField.from_dict(
            data={
                'type': 'entity_fk',
        ...
        self.assertIsInstance(deserialized, EntityFKField)
    ...
    def test_source__entity_fk__render1(self):
        instance_source = CreatedEntitySource(model=FakeContact)
        source = EntityFKField(
    ...
    def test_source__entity_fk__render2(self):
        user = self.get_root_user()
        orga = FakeOrganisation.objects.create(user=user, name='Acme')
        instance_source = FixedEntitySource(entity=orga)
        source = EntityFKField(entity_source=instance_source, field_name='image')
        self.assertEqual(
            _('Field «{field}» of: {source}').format(
                field=_('Logo'),
                source=instance_source.render(user=user, mode=source.TEXT_PLAIN),
    ...
    def test_source__first_related_entity__render1(self):
        rtype = RelationType.objects.get(id=REL_SUB_HAS)
    ...
    def test_source__first_related_entity__render2(self):
        user = self.get_root_user()
        rtype = RelationType.objects.get(id=REL_OBJ_HAS)
        orga = FakeOrganisation.objects.create(user=user, name='Acme')
