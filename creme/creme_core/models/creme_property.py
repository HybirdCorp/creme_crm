...
class CremePropertyTypeProxy:
    ...
    the ContentTypes add update the M2M when it's possible.
    ...
    But to update 'subject_ctypes', you have to use the methods 'add_ctypes()' &
    'remove_ctypes()'.
    ...
    def __init__(self,
        ...
        self._subject_ctypes = {as_ctype(ct_or_model) for ct_or_model in subject_ctypes}

    @property
    def subject_ctypes(self) -> Iterator[ContentType]:
        yield from self._subject_ctypes

    def add_ctypes(self,
                   *ctypes_or_models: ContentType | type[CremeEntity],
                   ) -> CremePropertyTypeProxy:
        self._subject_ctypes.update(
            as_ctype(ct_or_model) for ct_or_model in ctypes_or_models
        )

        return self

    def remove_ctypes(self,
                      *ctypes_or_models: ContentType | type[CremeEntity],
                      ) -> CremePropertyTypeProxy:
        for ct_or_model in ctypes_or_models:
            self._subject_ctypes.remove(as_ctype(ct_or_model))
    ...
    def __setattr__(self, name, value):
        if name in ('_instance', '_subject_ctypes'):
    ...
    def get_or_create(self) -> tuple[CremePropertyType, bool]:
        ...
        saved_instance.save()
        saved_instance.subject_ctypes.set(self._subject_ctypes)
    ...
    def update_or_create(self) -> CremePropertyType:
        ...
        instance.save()
        instance.subject_ctypes.set(self._subject_ctypes)
...
class CremePropertyTypeManager(models.Manager):
    ...
    def proxy(self,
        ...
        return CremePropertyTypeProxy(
            instance=self.model(**kwargs), subject_ctypes=subject_ctypes,
        )
