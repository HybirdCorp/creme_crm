...
class BlockDetailviewLocation(CremeModel):
    ...
    block_id     = CharField(max_length=100)  # TODO: rename brick_id
...
class BlockMypageLocation(CremeModel):
    ...
    # TODO: rename create_or_update ?
    # TODO: rename argument 'brick_id'
    @staticmethod
...
class RelationBlockItem(CremeModel):
    ...
    block_id       = CharField(_(u"Block ID"), max_length=100, editable=False)  # TODO: rename 'brick_id'
    ...
    def create(relation_type_id):
        try:
            ...
            rbi = RelationBlockItem.objects.create(
                        block_id=SpecificRelationsBrick.generate_id('creme_config',
                                                                    relation_type_id,
                                                                    ),
...
class InstanceBlockConfigItem(CremeModel):
    block_id = CharField(_(u'Brick ID'), max_length=300, blank=False, null=False, editable=False)  # TODO: rename 'brick_id'
    ...
    _block = None  # TODO: rename '_brick'
    ...
    def brick(self):
        ...
        if brick is None:
            from ..gui.bricks import brick_registry
            self._block = brick = brick_registry.get_brick_4_instance(self, entity=self.entity)
    ...
    @staticmethod
    def generate_id(block_class, entity, key):  # TODO: rename 'brick_class'
...
class BlockState(CremeModel):
    ...
    block_id          = CharField(_(u"Block ID"), max_length=100)  # TODO: rename 'brick_id'
    ...
    def get_for_brick_id(brick_id, user):
        ...
        try:
            return BlockState.objects.get(block_id=brick_id, user=user)
            ...
            return BlockState(block_id=brick_id, user=user,
    ...
    def get_for_brick_ids(brick_ids, user):
        ...
        for state in BlockState.objects.filter(block_id__in=brick_ids, user=user):
        ...
        for brick_id in set(brick_ids) - set(states.keys()):  # Bricks with unset state
            states[brick_id] = block_state(block_id=brick_id)
