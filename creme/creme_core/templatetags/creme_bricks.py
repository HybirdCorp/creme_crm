...
def brick_import(context, app=None, name=None, object=None, tag=None):
    ...
    if object is not None:
        ...
        brick_id = Brick.generate_id(app, name)
        brick = next(
            bricks.brick_registry.get_bricks(tag=tag, brick_ids=[brick_id]), None
        )
        if brick is None:
            raise LookupError(f'Cannot import the brick class with id="{brick_id}"')
