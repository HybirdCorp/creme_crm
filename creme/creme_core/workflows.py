...
class PropertyAddingTrigger(WorkflowTrigger):
    ...
    # TODO: factorise?
    def activate(self, event):
        if isinstance(event, PropertyAdded):
            prop = event.creme_property

            if (
                prop.type == self.property_type
    ...
    def from_dict(cls, data):
        # TODO: errors
        return cls(
            entity_model=_str_to_model(data['entity_model']),
            ptype=data['ptype'],
        )
    ...
    def to_dict(self):
        ...
        d['entity_model'] = _model_to_str(self._entity_model)
...
class TaggedEntitySource(_FromContextSource):
