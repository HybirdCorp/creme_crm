...
def model_from_key(model_key: str) -> type[Model]:
    ...
    try:
        ...
        raise WorkflowBrokenData(
            gettext('The model «{key}» is invalid').format(key=model_key)
        ) from e
...
class RelationAddingTrigger(WorkflowTrigger):
    ...
    def relation_type(self) -> RelationType:
        ...
        if rtype is False:
            raise WorkflowBrokenData(
                gettext('The relation type does not exist anymore')
            )
...
class _FromContextSource(WorkflowActionSource):
    ...
    @classmethod
    def from_dict(cls, data, registry) -> _FromContextSource:
        return cls(model=model_from_key(data['model']))
...
class PropertyAddingAction(WorkflowAction):
    ...
    # TODO: invalid ptype => log VS WorkflowResult (catch or raise WorkflowBrokenData?)
    # TODO: broken source => nothing? log? WorkflowResult?
    ...
    def render(self, user):
        ...
        try:
            ...
            return format_html(
                ...
                # error=gettext('the property type does not exist anymore'),
                error=e,
            )
...
class RelationAddingAction(WorkflowAction):
    ...
    # TODO: invalid rtype => log VS WorkflowResult (catch or raise WorkflowBrokenData?)
    # TODO: broken source => nothing? log? WorkflowResult?
    def execute(self, context):
    ...
    def render(self, user):
        try:
            ...
            return format_html(
                '{label}<p class="errorlist">{error}</p>',
                label=gettext('Adding a relationship'),
                # error=gettext('the relationship type does not exist anymore'),
                error=e,
            )
