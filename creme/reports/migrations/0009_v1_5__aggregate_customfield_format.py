...
class Migration(DataMigration):
    def forwards(self, orm):
        for rfield in orm['reports.Field'].objects.filter(type=RFT_AGGREGATE,
                                                          name__startswith='cf__',
                                                         ):
            try:
                #name: 'cf__%s__%s__max' % (cf.field_type, cf.id)
                prefix, cf_type, cf_id, agg_id = rfield.name.split('__')
            except ValueError:
                print 'The report field with id=%s is dubious & seems to no be related to a custom field (name=%s)' % (rfield.id, rfield.name)
            else:
                rfield.type = RFT_AGG_CUSTOM
                rfield.name = '%s__%s' % (cf_id, agg_id)
                rfield.save()

    def backwards(self, orm):
        CustomField = orm['creme_core.CustomField']
        cfields = CustomField.objects

        for rfield in orm['reports.Field'].objects.filter(type=RFT_AGG_CUSTOM):
            cf_id, agg_id = rfield.name.split('__')

            try:
                cfield = cfields.get(id=cf_id)
            except CustomField.DoesNotExist:
                print 'CustomField  id=%s does not exist anymore !? => remove ReportField id=%s' % (cf_id, rfield.id)
                rfield.delete()
            else:
                rfield.type = RFT_AGGREGATE
                rfield.name = 'cf__%s__%s' % (cfield.field_type, rfield.name)
                rfield.save()
