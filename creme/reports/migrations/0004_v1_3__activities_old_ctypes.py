...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for report in orm['reports.Report'].objects.filter(ct__in=[meeting_ct, pcall_ct, task_ct]):
            if report.filter:
                pass # work is done by 0005_v1_3__migrate_n_clean_old_activities_n_related
            else:
                report.filter = TYPE_MAP[report.ct_id]

            if report.ct_id == pcall_ct.id:
                for rfield in report.columns.filter(type=HFI_FIELD, name__startswith='call_type'):
                    #migration is not perfect: old call_type__description will migrate to invalid field
                    rfield.name = rfield.name.replace('call_type', 'sub_type')

                    fk_name, field_name = rfield.title.split(' - ')
                    rfield.title = u'%s - %s' % (_('Activity type'), field_name)

                    rfield.save()

            report.ct = activity_ct
            report.save()
