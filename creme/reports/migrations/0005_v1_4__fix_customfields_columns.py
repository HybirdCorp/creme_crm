...
class Migration(DataMigration):
    def forwards(self, orm):
        cfields_by_id   = dict(orm['creme_core.CustomField'].objects.values_list('id', 'name'))
        cfields_by_name = dict((cf_name, cf_id) for cf_id, cf_name in cfields_by_id.iteritems())

        for rfield in orm['reports.Field'].objects.filter(type=HFI_CUSTOM):
            name = rfield.name

            if name.isdigit() and int(name) in cfields_by_id:
                continue #Field.name == CustomField.id   => OK

            cf_id = cfields_by_name.get(name)

            if cf_id is None: #CustomField has been edited/removed
                print u"Unknown CustomField '%s' => delete the Report column" % name
                rfield.report_columns_set.clear()
                rfield.delete()
            else:
                rfield.name = cf_id
                rfield.save()
