    ...
    def detailview_display(self, context):
        ...
        for chart in charts:
            ...
            context['rows'].append({
                'chart': chart,
                'data': data,
                'instance_brick_count': counter[chart.id],
                'settings_update_url': reverse(
...
class ReportChartInstanceBrick(ReportChartMixin, core_bricks.InstanceBrick):
    ...
    def __init__(self, instance_brick_config_item):
        super().__init__(instance_brick_config_item)
        get_data = instance_brick_config_item.get_extra_data
        # report = instance_brick_config_item.entity.get_real_entity()
        # TODO: manage error
        chart = ReportChart.objects.get(uuid='TODO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')

        fetcher = self.fetcher = ReportChart.fetcher_registry.get(
            chart=chart,
            fetcher_dict={
                key: get_data(key)
                for key in ChartFetcher.DICT_KEYS
            },
        )
    ...
    def _render_chart(self, context, chart, data, **kwargs):
        context = self.get_template_context(
            ...
            settings_update_url=reverse(
                ...
                args=(self.config_item.id, chart.id,)
            ),
            plots=[*plot_registry],
            props={
                plot.name: plot.props(chart=chart, data=data)
    ...
    def home_display(self, context):
        ...
        x, y = fetcher.fetch(
            user=user,
            order='ASC' if fetcher.chart.asc else 'DESC',
        )
        return self._render_chart(
            context,
            chart=fetcher.chart,
...
class ReportChartBrick(ReportChartMixin, core_bricks.Brick):
    ...
    def detailview_display(self, context):
        ...
        return self._render(self.get_template_context(
            ...
            plots=[*plot_registry],
            props={
                plot.name: chart.props(chart=chart, data=data)
                for plot in plot_registry
            },
        ))
