...
from .aggregator import AGGREGATORS_MAP, ReportChartAggregator
...
class ChartHand:
    ...
    def __init__(self, chart: ReportChart):
        self._chart = chart
        self._y_calculator = y_calculator = AGGREGATORS_MAP[chart]
    ...
    def _listview_url_builder(self, extra_q: Q | None = None):
        ...
        return ListViewURLBuilder(
            model=chart.model,
            filter=chart.linked_report.filter,
            common_q=extra_q,
        )
    ...
    @property
    def ordinate(self) -> ReportChartAggregator:
...
class ChartHandRegistry:
    __slots__ = ('_hands', )

    def __init__(self) -> None:
        self._hands: dict[int, type[ChartHand]] = {}

    def __call__(self, hand_id: int):
        assert hand_id not in self._hands, 'ID collision'

        def _aux(cls: type[ChartHand]):
    ...
    def __getitem__(self, i: int) -> type[ChartHand]:
        return self._hands[i]

    def __iter__(self) -> Iterator[int]:
        return iter(self._hands)

    def get(self, i: int) -> type[ChartHand] | None:
        return self._hands.get(i)


CHART_HANDS_MAP = ChartHandRegistry()


class _ChartHandRegularField(ChartHand):
    def __init__(self, chart):
        super().__init__(chart)

        cell = chart.abscissa_info.cell
        if cell is None:
            field = None
...
@CHART_HANDS_MAP(AbscissaGroup.DAY)
class ChartHandDay(_ChartHandRegularField):
...
@CHART_HANDS_MAP(AbscissaGroup.MONTH)
class ChartHandMonth(_ChartHandRegularField):
...
@CHART_HANDS_MAP(AbscissaGroup.YEAR)
class ChartHandYear(_ChartHandRegularField):
...
class _DateRangeMixin:
    ...
    def get_days(chart):
        try:
            days = int(chart.abscissa_info.parameter)
        except (TypeError, ValueError) as e:
            logger.warning('Invalid chart days parameter "%s"', e)
            days = 1

        return days


@CHART_HANDS_MAP(AbscissaGroup.RANGE)
class ChartHandRange(_DateRangeMixin, _ChartHandRegularField):
    ...
    def __init__(self, chart):
        ...
        if vendor not in {'sqlite', 'mysql', 'postgresql'}:
            logger.warning(
                'Chart data optimizations not available with '
                'DB vendor "%s", reverting to slower fallback method.',
                vendor,
            )
            self._fetch_method = self._fetch_fallback

        self._days = self.get_days(chart)
    ...
    def _fetch_fallback(self, entities, order, extra_q):
        ...
        if min_date is not None and max_date is not None:
            ...
            for interval in DateInterval.generate(
                (chart.days or 1) - 1, min_date, max_date, order,
            ):
...
@CHART_HANDS_MAP(AbscissaGroup.FK)
class ChartHandForeignKey(_ChartHandRegularField):
...
@CHART_HANDS_MAP(AbscissaGroup.CHOICES)
class ChartHandChoices(_ChartHandRegularField):
...
@CHART_HANDS_MAP(AbscissaGroup.RELATION)
class ChartHandRelation(ChartHand):
    ...
    def _fetch(self, *, entities, order, user, extra_q):
        ...
        for obj_id, obj_str in Relation.objects.filter(
            type_id=rtype_id, subject_entity__entity_type=self._chart.linked_report.ct,
        ).values_list(
            'object_entity_id',
...
class _ChartHandCustomField(ChartHand):
    def __init__(self, chart):
        super().__init__(chart)
...
@CHART_HANDS_MAP(AbscissaGroup.CUSTOM_DAY)
class ChartHandCustomDay(_ChartHandCustomField):
    verbose_name = _('By days')
...
@CHART_HANDS_MAP(AbscissaGroup.CUSTOM_MONTH)
class ChartHandCustomMonth(_ChartHandCustomField):
    verbose_name = _('By months')
...
@CHART_HANDS_MAP(AbscissaGroup.CUSTOM_YEAR)
class ChartHandCustomYear(_ChartHandCustomField):
    verbose_name = _('By years')
...
@CHART_HANDS_MAP(AbscissaGroup.CUSTOM_RANGE)
class ChartHandCustomRange(_DateRangeMixin, _ChartHandCustomField):
    verbose_name = _('By X days')
    ...
    def __init__(self, chart):
        ...
        if vendor not in {'sqlite', 'mysql', 'postgresql'}:
            logger.warning(
                'Report chart data optimizations not available with '
                'DB vendor "%s", reverting to slower fallback method.',
                vendor,
            )
            self._fetch_method = self._fetch_fallback
    ...
    def _fetch_fallback(self, entities, order, extra_q):
        ...
        if min_date is not None and max_date is not None:
            ...
            for interval in DateInterval.generate(
                    (self._chart.days or 1) - 1, min_date, max_date, order,
            ):
                before = interval.before
                after  = interval.after
...
@CHART_HANDS_MAP(AbscissaGroup.CUSTOM_FK)
class ChartHandCustomFK(_ChartHandCustomField):
