...
class CHCCRegularField(ChartHandCellConstraint):
...
class CHCCRegularFK(CHCCRegularField):
    type_id = 'regular_fk'
...
class CHCCRegularChoices(CHCCRegularField):
...
class CHCCRegularDate(CHCCRegularField):
...
class CHCCRelation(ChartHandCellConstraint):
    type_id = 'rtype'
...
class CHCCCustomField(ChartHandCellConstraint):
...
class CHCCCustomEnum(CHCCCustomField):
    type_id = 'custom_enum'
    customfield_types = {CustomField.ENUM}


class CHCCCustomDate(CHCCCustomField):
    type_id = 'custom_date'
    customfield_types = {CustomField.DATE, CustomField.DATETIME}


class ChartHandConstraintsRegistry:
    class RegistrationError(Exception):
        pass

    def __init__(self) -> None:
        self._constraints_by_type_id: dict[str, type[ChartHandCellConstraint]] = {}
        self._constraints_by_chart_type:  dict[int, type[ChartHandCellConstraint]] = {}
        self._param_validators: dict[int, forms.Field] = {}

    def cell_constraints(self,
                         model: type[CremeEntity],
                         ) -> Iterator[ChartHandCellConstraint]:
        for constraint_class in self._constraints_by_type_id.values():
            yield constraint_class(model)

    def get_constraint_by_chart_type(self,
                                     model: type[CremeEntity],
                                     chart_type: int,
                                     ) -> ChartHandCellConstraint | None:
        constraint_class = self._constraints_by_chart_type.get(chart_type)
        return constraint_class(model) if constraint_class else None

    def get_parameter_validator(self, chart_type: int) -> forms.Field | None:
        return self._param_validators.get(chart_type)
    ...
    def register_cell_constraint(self, *,
        ...
        for chart_type in chart_types:
            ...
            if set_constr_by_chart_type(chart_type, constraint_class) is not constraint_class:
                raise self.RegistrationError(
                    f'{type(self).__name__}.register_cell_constraint(): '
                    f'a constraint is already registered for the chart-type "{chart_type}".'
                )
    ...
    def register_parameter_validator(self, *,
        ...
        for chart_type in chart_types:
            if set_validator(chart_type, formfield) is not formfield:
                raise self.RegistrationError(
                    f'{type(self).__name__}.register_parameter_validator(): '
                    f'the validator "{formfield}" is already registered for '
                    f'the type "{chart_type}".'
                )

        return self

    @property
    def chart_types(self) -> Iterator[int]:
        return iter(self._constraints_by_chart_type.keys())


abscissa_constraints = ChartHandConstraintsRegistry(
).register_cell_constraint(
    constraint_class=CHCCRegularFK,
    chart_types=[AbscissaGroup.FK],
).register_cell_constraint(
    constraint_class=CHCCRegularChoices,
    chart_types=[AbscissaGroup.CHOICES],
).register_cell_constraint(
    ...
    constraint_class=CHCCCustomEnum,
    chart_types=[AbscissaGroup.CUSTOM_FK],
).register_cell_constraint(
    constraint_class=CHCCCustomDate,
    chart_types=[
