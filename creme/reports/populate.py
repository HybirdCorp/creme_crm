...
class Populator(BasePopulator):
    ...
    def populate(self, *args, **kwargs):
        ...
        if 'creme.opportunities' in settings.INSTALLED_APPS:
            ...
            opp_ct_id = get_ct(Opportunity).id

            #Create a list view filter to use it in the report
            opp_filter = create(Filter, name=_(u"Generated by a creme managed organisation"), model_ct_id=opp_ct_id, is_custom=False)
            ...
            #Create the report
            opp_report = create(Report, name=_(u"Opportunities generated by a creme managed organisation"), ct_id=opp_ct_id, filter_id=opp_filter.id, user_id=admin_pk)
            opp_report_columns = []
            ...
            #Create 2 graphs
            opp_report_graph_1 = create(ReportGraph, name=_(u"Sum %(estimated_sales)s / %(sales_phase)s") % {'estimated_sales': get_verbose_field_name(Opportunity, 'estimated_sales'), 'sales_phase': get_verbose_field_name(Opportunity, 'sales_phase')}, report_id=opp_report.id, abscissa='sales_phase', ordinate='estimated_sales__sum', type=RGT_FK, is_count=False, user_id=admin_pk)
            opp_report_graph_2 = create(ReportGraph, name=_(u"Sum %(estimated_sales)s / Quarter (90 days on %(closing_date)s)") % {'estimated_sales': get_verbose_field_name(Opportunity, 'estimated_sales'), 'closing_date': get_verbose_field_name(Opportunity, 'closing_date')}, report_id=opp_report.id, abscissa='closing_date', ordinate='estimated_sales__sum', type=RGT_RANGE, is_count=False, user_id=admin_pk, days=90)

            #Create 2 instance block items for the 2 graphs
#            opp_rgraph1_vcolumn = '%s#%s' % (REL_OBJ_EMIT_ORGA, HFI_RELATION)
            rgraph_1_instance_block = create(InstanceBlockConfigItem, entity_id=opp_report_graph_1.id, block_id=ReportGraphBlock.generate_id('creme_config', u"%s_" % (opp_report_graph_1.id, )), verbose = u"%s - %s" % (opp_report_graph_1, _(u'None')), data='')
            rgraph_2_instance_block = create(InstanceBlockConfigItem, entity_id=opp_report_graph_2.id, block_id=ReportGraphBlock.generate_id('creme_config', u"%s_" % (opp_report_graph_2.id, )), verbose = u"%s - %s" % (opp_report_graph_2, _(u'None')), data='')
