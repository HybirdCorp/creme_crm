...
class ChartFetcherTestCase(CremeTestCase):
    def test_simple(self):
        ...
        fetcher1 = SimpleChartFetcher(graph=graph)
        ...
        # ---
        fetcher2 = SimpleChartFetcher(graph=graph, value='last_name')
        ...
        self.assertEqual(graph, b_fetcher.chart)
    ...
    def test_fk02(self):
        ...
        user = self.get_root_user()
        report = Report.objects.create(user=user, name='Field Test', ct=FakeContact)
        chart = ReportChart(user=user, name='Field Test', linked_report=report)

        fetcher = RegularFieldLinkedChartFetcher(graph=chart, value=hidden_fname)
    ...
    def test_relation(self):
        ...
        fetcher1 = RelationLinkedChartFetcher(graph=chart, value=FAKE_REL_SUB_EMPLOYED_BY)
        self.assertIsNone(fetcher1.error)
        self.assertEqual(
        ...
        fetcher2 = RelationLinkedChartFetcher(graph=chart)
        self.assertEqual(
        ...
        fetcher3 = RelationLinkedChartFetcher(graph=chart, value='invalid')
        self.assertEqual(
            _('The relationship type is invalid.'),
            fetcher3.error
        )

        fetcher4 = RelationLinkedChartFetcher(graph=chart, value=FAKE_REL_SUB_BILL_ISSUED)
        self.assertEqual(
    ...
    def test_create_brick_config_item(self):
        "Other brick class."
        class OtherReportChartBrick(ReportGraphChartInstanceBrick):
...
class ChartFetcherRegistryTestCase(CremeTestCase):
    ...
    def test_register02(self):
        ...
        class OtherFKGraphFetcher(RegularFieldLinkedChartFetcher):
            pass
