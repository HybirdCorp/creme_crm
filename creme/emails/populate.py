...
class Populator(BasePopulator):
    ...
    def populate(self):
        ...
        if not already_populated:
            ...
            create_bdl(block_id=bricks.EmailHTMLBodyBrick.id_,     order=20,  zone=LEFT,  model=EntityEmail)
            create_bdl(block_id=core_bricks.CustomFieldsBrick.id_, order=40,  zone=LEFT,  model=EntityEmail)
            create_bdl(block_id=core_bricks.PropertiesBrick.id_,   order=450, zone=LEFT,  model=EntityEmail)
            create_bdl(block_id=core_bricks.RelationsBrick.id_,    order=500, zone=LEFT,  model=EntityEmail)
            create_bdl(block_id=core_bricks.HistoryBrick.id_,      order=20,  zone=RIGHT, model=EntityEmail)

            BlockDetailviewLocation.create_4_model_brick(          order=5,   zone=LEFT,  model=MailingList)
            create_bdl(block_id=core_bricks.CustomFieldsBrick.id_, order=40,  zone=LEFT,  model=MailingList)
            create_bdl(block_id=bricks.EmailRecipientsBrick.id_,   order=80,  zone=LEFT,  model=MailingList)
            create_bdl(block_id=bricks.ContactsBrick.id_,          order=90,  zone=LEFT,  model=MailingList)
            create_bdl(block_id=bricks.OrganisationsBrick.id_,     order=95,  zone=LEFT,  model=MailingList)
            create_bdl(block_id=bricks.ChildListsBrick.id_,        order=100, zone=LEFT,  model=MailingList)
            create_bdl(block_id=bricks.ParentListsBrick.id_,       order=105, zone=LEFT,  model=MailingList)
            create_bdl(block_id=core_bricks.PropertiesBrick.id_,   order=450, zone=LEFT,  model=MailingList)
            create_bdl(block_id=core_bricks.RelationsBrick.id_,    order=500, zone=LEFT,  model=MailingList)
            create_bdl(block_id=core_bricks.HistoryBrick.id_,      order=20,  zone=RIGHT, model=MailingList)

            create_bdl(block_id=bricks.SendingsBrick.id_,          order=2,   zone=TOP,   model=EmailCampaign)
            BlockDetailviewLocation.create_4_model_brick(          order=5,   zone=LEFT,  model=EmailCampaign)
            create_bdl(block_id=core_bricks.CustomFieldsBrick.id_, order=40,  zone=LEFT,  model=EmailCampaign)
            create_bdl(block_id=bricks.MailingListsBrick.id_,      order=120, zone=LEFT,  model=EmailCampaign)
            create_bdl(block_id=core_bricks.PropertiesBrick.id_,   order=450, zone=LEFT,  model=EmailCampaign)
            ...
            create_bdl(block_id=bricks.TemplateHTMLBodyBrick.id_,  order=70,  zone=LEFT,  model=EmailTemplate)
            create_bdl(block_id=core_bricks.PropertiesBrick.id_,   order=450, zone=LEFT,  model=EmailTemplate)
            create_bdl(block_id=core_bricks.RelationsBrick.id_,    order=500, zone=LEFT,  model=EmailTemplate)
            create_bdl(block_id=core_bricks.HistoryBrick.id_,      order=20,  zone=RIGHT, model=EmailTemplate)

            # 'persons' app
            create_bdl(block_id=bricks.MailsHistoryBrick.id_, order=600, zone=RIGHT, model=Contact)
            create_bdl(block_id=bricks.MailsHistoryBrick.id_, order=600, zone=RIGHT, model=Organisation)

            BlockPortalLocation.create(app_name='emails', block_id=bricks.SignaturesBrick.id_, order=10)
            BlockPortalLocation.create(app_name='emails', block_id=core_bricks.HistoryBrick.id_, order=30)
            ...
            if apps.is_installed('creme.assistants'):
                ...
                BlockPortalLocation.create(app_name='emails', block_id=a_bricks.MemosBrick.id_,        order=100)
                BlockPortalLocation.create(app_name='emails', block_id=a_bricks.AlertsBrick.id_,       order=200)
                BlockPortalLocation.create(app_name='emails', block_id=a_bricks.UserMessagesBrick.id_, order=300)
            ...
            if apps.is_installed('creme.documents'):
                ...
                create_bdl(block_id=LinkedDocsBrick.id_, order=600, zone=RIGHT, model=EmailCampaign)
