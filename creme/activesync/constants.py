...
MAPI_SERVER_URL = "activesync-mapi-server_url"
MAPI_DOMAIN     = "activesync-mapi-domain"
MAPI_SERVER_SSL = "activesync-mapi-server_ssl"
...
SYNC_FOLDER_TYPE_TASK             = 7  # Task
SYNC_FOLDER_TYPE_APPOINTMENT      = 8  # Meeting
SYNC_FOLDER_TYPE_CONTACT          = 9  # Contact
...
SYNC_AIRSYNC_STATUS_SERVER_ERR           = 5   # Server misconfiguration, temporary system issue, or bad item.
                                               #  This is frequently a transient condition.
SYNC_AIRSYNC_STATUS_CLIENT_SERV_CONV_ERR = 6   # The client has sent a malformed or invalid item.
SYNC_AIRSYNC_STATUS_MATCHING_CONFLICT    = 7   # The client has changed an item for which the conflict policy indicates
                                               #  that the server's changes take precedence.
SYNC_AIRSYNC_STATUS_OBJ_NOT_FOUND        = 8   # The client issued a <Fetch> or <Change> operation that has an ItemID value
...
SYNC_AIRSYNC_STATUS_SYNC_UNCOMPLETE      = 13  # The Sync command request is not complete. An empty or partial Sync command request
                                               #  is received and the cached set of notify- able collections is missing
SYNC_AIRSYNC_STATUS_INVALID_TIME_OPTIONS = 14  # The Sync request was processed successfully but the <Wait> or <HeartbeatInterval>
                                               # interval that is specified by the client is outside the range set by the server
                                               # administrator. If the <HeartbeatInterval> or <Wait> value included in the Sync
                                               # request is larger than the maximum allowable value, the response contains a
                                               #  <Limit> element that specifies the maximum allowed value.
                                               # If the <HeartbeatInterval> or <Wait> value included in the Sync request is
                                               # smaller than the minimum allowable value, the response contains a <Limit>
                                               # element that specifies the minimum allowed value.
