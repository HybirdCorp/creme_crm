...
class TriggerStep(_WorkflowWizardFormStep):
    ...
    def clean(self):
        ...
        if not self._errors:
            # [0] == trigger class ID
            self.instance.trigger = self.cleaned_data['trigger'][1]
...
class FirstActionSelectionStep(_WorkflowWizardFormStep):
    ...
    #
    #     if not self._errors:
    #         self.instance.wizard_action_class = self.cleaned_data['action_type']
    #
    #     return cdata
...
class FirstActionCreationStep(_WorkflowWizardFormStep):
    pass
    # action_type = forms.ChoiceField(
    #     label=_('Type of action'),
    #     # help_text=TODO
