...
class GenericPortalsTestCase(BrickTestCaseMixin, CremeTestCase):
    ...
    def test_reload_app_bricks(self):
    ...
    def test_reload_app_bricks__custom_brick(self):
        response = self.assertGET200(
...
class GenericCreationTestCase(CremeTestCase):
    def setUp(self):
        super().setUp()
        self.login_as_root()
    ...
    def test_from_widget__order(self):
        count = FakeSector.objects.count()
...
class GenericEditionTestCase(CremeTestCase):
    def test_simple(self):
        self.login_as_root()
    ...
    def test_order_no_changed(self):
        self.login_as_root()
    ...
    def test_deletion_in_progress(self):
        ...
        sector1 = FakeSector.objects.create(title='music')

        DeletionCommand.objects.create(
            content_type=FakeSector,
            job=Job.objects.create(type_id=deletor_type.id, user=user),
            pk_to_delete=str(sector1.pk),
            deleted_repr=str(sector1),
        )

        response = self.assertGET409(reverse(
            'creme_config__edit_instance',
            args=('creme_core', 'fake_sector', sector1.id),
        ))
        self.assertContains(
            response,
            text=_('This object is being deleted.'),
            status_code=409,
        )

        # ---
        sector2 = FakeSector.objects.create(title='Hokage')
        self.assertGET200(
            reverse(
                'creme_config__edit_instance',
                args=('creme_core', 'fake_sector', sector2.pk),
            ),
        )


class GenericDisablingTestCase(CremeTestCase):
    def test_on(self):
        self.login_as_root()
    ...
    def test_off__deletion_in_progress(self):
        user = self.login_as_root_and_get()
        pos1 = FakePosition.objects.create(title='Kunoichi', disabled=now())

        DeletionCommand.objects.create(
            content_type=FakePosition,
            job=Job.objects.create(type_id=deletor_type.id, user=user),
            pk_to_delete=str(pos1.pk),
            deleted_repr=str(pos1),
        )

        response = self.assertPOST409(
            reverse(
                'creme_config__disable_instance',
                args=('creme_core', 'fake_position', pos1.pk),
            ),
            data={'action': 'enable'},
        )
        self.assertContains(
        ...
        # ---
        pos2 = FakePosition.objects.create(title='Hokage', disabled=now())

        # NB: should not stop the deletion
        sector = FakeSector.objects.get_or_create(
            pk=pos2.pk, defaults={'title': 'Annoying sector'},
        )[0]
        DeletionCommand.objects.create(
            content_type=FakeSector,
            job=Job.objects.create(type_id=deletor_type.id, user=user),
            pk_to_delete=str(sector.pk),
            deleted_repr=str(sector),
        )

        self.assertPOST200(
            reverse(
                'creme_config__disable_instance',
                args=('creme_core', 'fake_position', pos2.pk),
            ),
            data={'action': 'enable'},
        )
...
class GenericDeletionTestCase(CremeTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_set_null(self):
        ...
        # No related entity ---
        get_response1 = self.assertGET200(url)

        context = get_response1.context
        self.assertEqual(
        ...
        get_response2 = self.assertGET200(url)
        self.assertEqual(
            ngettext(
            ...
            self.get_form_or_fail(get_response2).fields[fname].initial
        )

        # Two related entity ---
        contact2 = create_contact(last_name='Hattori', first_name='Hanzo')

        get_response3 = self.assertGET200(url)
        self.assertEqual(
            ngettext(
                'BEWARE: {count} instance of «{model}» uses «{instance}» & '
                'will be updated (the field will be emptied).',
            ...
            self.get_form_or_fail(get_response3).fields[fname].initial,
        )
        ...
        self.assertDatetimesAlmostEqual(pos2del.disabled, now())
    ...
    def test_replace(self):
        """Several fields + replacement + limit_choices + on_delete=CREME_REPLACE."""
    ...
    def test_protect__no_related(self):
        """on_delete=PROTECT + no related entity."""
        self.assertIs(
    ...
    def test_model_with_m2m(self):
        """Deleted model gets a M2M field (bugfix).
        + no 'disabled' field.
        """
    ...
    def test_hidden_fields__set_null(self):
        """on_delete=SET_NULL."""
        self.assertIs(
    ...
    def test_hidden_fields__set(self):
        """on_delete=SET."""
        FieldsConfig.objects.create(
    ...
    def test_hidden_fields__set_default(self):
        """on_delete=SET_DEFAULT."""
        FieldsConfig.objects.create(
            content_type=FakeTicket,
    ...
    def test_hidden_fields__cascade(self):
        """on_delete=CASCADE."""
        FieldsConfig.objects.create(
    ...
    def test_hidden_fields__protect__no_related(self):
        """on_delete=PROTECT + no related entity."""
        FieldsConfig.objects.create(
    ...
    def test_hidden_fields__protect__related(self):
        """on_delete=PROTECT + related entity."""
        FieldsConfig.objects.create(
            content_type=FakeActivity,
            descriptions=[('type', {FieldsConfig.HIDDEN: True})],
    ...
    def test_hidden_fields__replace_null(self):
        """on_delete=CREME_REPLACE_NULL."""
        FieldsConfig.objects.create(
            content_type=FakeContact,
    ...
    def test_hidden_fields__replace__no_related(self):
        """on_delete=CREME_REPLACE + no related entity."""
        FieldsConfig.objects.create(
    ...
    def test_hidden_fields__replace__related(self):
        """on_delete=CREME_REPLACE + some related entity."""
        FieldsConfig.objects.create(
            content_type=FakeOrganisation,
            descriptions=[('sector', {FieldsConfig.HIDDEN: True})],
        )
    ...
    def test_efilter_conditions__replace(self):
        ...
        sector1 = create_sector(title='Bo')
        sector2 = create_sector(title='Blade')
        sector3 = create_sector(title='Nunchaku')
        sector_to_del = create_sector(title='Gun')

        efilter_to_update1 = EntityFilter.objects.smart_update_or_create(
        ...
        efilter_to_update2 = EntityFilter.objects.smart_update_or_create(
            pk='test-filter_using_replaced_deep', name='Related is not armed',
            model=FakeResource, is_custom=True,
            conditions=[
                condition_handler.RegularFieldConditionHandler.build_condition(
                    model=FakeResource,
                    operator=operators.EqualsNotOperator,
                    field_name='linked_contact__sector',
                    values=[str(sector_to_del.uuid), str(sector3.uuid)],
                ),
            ],
        )
        ...
        self.assertDictEqual(
            ...
            efilter_to_update1.get_conditions()[0].value,
        )
        ...
        self.assertEqual(2, self.refresh(dcom).updated_count)

        # efilter1 --
        condition_value1 = self.get_alone_element(
            self.refresh(efilter_to_update1).get_conditions()
        ).value
        self.assertIsDict(condition_value1, length=2)
        self.assertEqual('equals', condition_value1.get('operator'))
        self.assertCountEqual(  # NB: order of keys may have changed
            [str(sector1.uuid), str(sector2.uuid)], condition_value1.get('values'),
        )

        # efilter2 --
        condition_value2 = self.get_alone_element(
            self.refresh(efilter_to_update2).get_conditions()
        ).value
        self.assertIsDict(condition_value2, length=2)
        self.assertEqual('equals', condition_value1.get('operator'))
        self.assertCountEqual(  # NB: order of keys may have changed
            [str(sector1.uuid), str(sector3.uuid)], condition_value2.get('values'),
        )
    ...
    def test_blocking_soft_ref__credential_filter__no_filter(self):
        ...
        EntityFilter.objects.create(
            ...
            [condition_handler.RegularFieldConditionHandler.build_condition(
                ...
                operator=operators.EqualsOperator,
                field_name='position',
                values=[str(position.uuid)],  # <=== same as sector_to_del
                filter_type=EF_CREDENTIALS,
            )],
            check_cycles=False, check_privacy=False,
        )
    ...
    def test_customisation__disabled(self):
        """Deletion disabled
        (see creme.creme_core.apps.CremeCoreConfig.register_creme_config()).
        """
...
class DeletorEndTestCase(CremeTestCase):
    ...
    def test_with_errors__no_message(self):
    ...
    def test_with_errors__message(self):
...
class GenericReorderingTestCase(CremeTestCase):
