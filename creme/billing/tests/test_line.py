...
from creme.creme_core.core.workflow import EntityCreated, WorkflowEngine
...
class LineTestCase(BrickTestCaseMixin, _BillingTestCase):
    ...
    def test_event_line_updated(self):
        user = self.login_as_root_and_get()
        invoice1, emitter, receiver  = self.create_invoice_n_orgas(user=user, name='Invoice001')
        line11 = ProductLine.objects.create(
            user=user, related_document=invoice1, on_the_fly_item='Flyyyyy',
        )
        line12 = ServiceLine.objects.create(
            user=user, related_document=invoice1, on_the_fly_item='Flyyyyy',
        )

        event11 = LineUpdated(line=line11)
        event12 = LineUpdated(line=line12)
        self.assertTrue(event11.inhibits(event12))

        other_type_event = EntityCreated(entity=emitter)
        self.assertFalse(event11.inhibits(other_type_event))

        invoice2  = self.create_invoice(
            user=user, name='Invoice002', source=emitter, target=receiver
        )
        line21 = ProductLine.objects.create(
            user=user, related_document=invoice2, on_the_fly_item='Flyyyyy',
        )
        event21 = LineUpdated(line=line21)
        self.assertFalse(event11.inhibits(event21))

    def test_clean__percent(self):
        "Discount.PERCENT."
        user = self.login_as_root_and_get()
    ...
    def test_clean__line_amount(self):
    ...
    def test_clean__item_amount(self):
    ...
    def test_clean__with_related_item(self):
        user = self.login_as_root_and_get()
        invoice = self.create_invoice_n_orgas(user=user, name='Invoice01', discount=0)[0]
    ...
    @skipIfCustomProduct
    def test_add_lines__product(self):
        "Multiple adding."
        user = self.login_as_root_and_get()
    ...
    @skipIfCustomService
    def test_add_lines__service(self):
        "Multiple adding."
        user = self.login_as_root_and_get()
    ...
    def test_add_lines__not_superuser(self):
    ...
    def test_add_lines__bad_related(self):
    ...
    @skipIfCustomProductLine
    def test_related_item__on_the_fly_product(self):
        user = self.login_as_root_and_get()
    ...
    @skipIfCustomProductLine
    def test_related_item__cache(self):
        # Fill caches
        Vat.objects.default()
    ...
    @skipIfCustomProductLine
    def test_delete_vat__blocked(self):
        user = self.login_as_root_and_get()
    ...
    @skipIfCustomServiceLine
    def test_add_to_catalog__service(self):
    ...
    @skipIfCustomProductLine
    def test_add_to_catalog__no_product_creation_perm(self):
    ...
    @skipIfCustomServiceLine
    def test_add_to_catalog__no_service_creation_perm(self):
    ...
    @skipIfCustomProductLine
    def test_add_to_catalog__already_related_product(self):
    ...
    @skipIfCustomServiceLine
    def test_add_to_catalog__already_related_service(self):
    ...
    @skipIfCustomProductLine
    def test_multi_save_lines__validation_errors__no_saved_line(self):
        "No saved line."
        user = self.login_as_root_and_get()
    ...
    @skipIfCustomProductLine
    def test_multi_save_lines__validation_errors__non_field_error(self):
    ...
    @skipIfCustomProductLine
    def test_multi_save_lines__order(self):
        user = self.login_as_root_and_get()
    ...
    @skipIfCustomProductLine
    def test_discount__document_s_discount(self):
        "Document's discount."
        user = self.login_as_root_and_get()
