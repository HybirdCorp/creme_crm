...
@skipIfCustomInvoice
class MiscLineViewsTestCase(base.BrickTestCaseMixin,
...
class LinesCreationTestCase(base.BrickTestCaseMixin, _BillingTestCase):
    ...
    def test_service(self):
        ...
        with self.assertNoException():
            discount_f = context['form'].fields['discount']
            sub_decimal_f = discount_f.fields[1]
            max_value = sub_decimal_f.max_value
            min_value = sub_decimal_f.min_value
        self.assertEqual(Decimal('100'), max_value)
        self.assertEqual(Decimal('0'),   min_value)
...
@skipIfCustomOrganisation
@skipIfCustomInvoice
class AddingToCatalogTestCase(_BillingTestCase):
    @staticmethod
    ...
    @skipIfCustomProductLine
    def test_creation_perms__product(self):
    ...
    @skipIfCustomServiceLine
    def test_creation_perms__service(self):
    ...
    @skipIfCustomProductLine
    def test_already_related__product(self):
    ...
    @skipIfCustomServiceLine
    def test_already_related__service(self):
...
@skipIfCustomOrganisation
@skipIfCustomInvoice
class MultiLinesSavingTestCase(base.BrickTestCaseMixin, _BillingTestCase):
    @staticmethod
    def _build_msave_url(bdocument):
        return reverse('billing__multi_save_lines', args=(bdocument.id,))

    @skipIfCustomServiceLine
    def test_one_edition(self):
    ...
    @skipIfCustomProductLine
    def test_one_creation_and_one_deletion(self):
    ...
    @skipIfCustomServiceLine
    def test_discount__amount_per_line(self):
    ...
    @skipIfCustomServiceLine
    def test_discount__amount_per_item(self):
    ...
    @skipIfCustomProductLine
    def test_validation_errors__no_saved_line(self):
        user = self.login_as_root_and_get()
    ...
    @skipIfCustomProductLine
    def test_validation_errors__edited_line(self):
...
class LinesReorderingTestCase(_BillingTestCase):
    ...
    @skipIfCustomProductLine
    def test_ok(self, target_name, next_order, expected):
        user = self.login_as_root_and_get()
        invoice = self.create_invoice_n_orgas(user=user, name='Invoice001', discount=0)[0]
