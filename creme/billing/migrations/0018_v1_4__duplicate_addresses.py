...
class Migration(DataMigration):
    def forwards(self, orm):
        Address = orm['persons.Address']
        FIELD_NAMES = ('name', 'address', 'po_box', 'zipcode', 'city', 'department', 'state', 'country')

        def clone_address(address, entity):
            return Address.objects.create(object_id=entity.id,
                                          content_type=entity.entity_type,
                                          **dict((fname, getattr(address, fname)) for fname in FIELD_NAMES)
                                         )

        for base in orm['billing.Base'].objects.exclude(Q(billing_address__object_id=F('id')) &
                                                        Q(shipping_address__object_id=F('id'))
                                                       ):
            if base.billing_address is not None and base.billing_address.object_id != base.id:
               base.billing_address = clone_address(base.billing_address, base)

            if base.shipping_address is not None and base.shipping_address.object_id != base.id:
               base.shipping_address = clone_address(base.shipping_address, base)
