...
class BulkExport(base.EntityCTypeRelatedMixin, base.CheckedView):
    permissions = 'billing'
    # entity_classes = [ TODO (improve EntityCTypeRelatedMixin)
    #     billing.get_invoice_model(),
    #     billing.get_credit_note_model(),
    #     billing.get_quote_model(),
    #     billing.get_sales_order_model(),
    ...
    def get(self, request, *args, **kwargs):
        ...
        entities = EntityCredentials.filter(
            user=user, queryset=model.objects.filter(id__in=raw_ids),
        )
        # NB: we should remove duplicate before...
        if len(entities) != len(cleaned_ids):
            raise PermissionDenied(_('Some entities are invalid or not viewable'))

        # TODO: factorise with other view
        config_item = get_object_or_404(ExporterConfigItem, content_type=ctype)
        ...
        with ZipFile(final_path, 'w') as archive:
            for entity in entities:
                ...
                if isinstance(export_result, HttpResponse):
                    # TODO: extract name (it could be .xls etc...)
                    #  import re
                    #  disp_re = re.compile('attachment; filename="(?P<name>.+)"')
                    #  file_name_match = disp_re.match(export_result['Content-Disposition'])
                    #  if file_name_match is None:
                    #      ...
                    #  else:
                    #      fname = file_name_match['name']
                    #      ...
                    archive.writestr(
                        f'{entity._meta.verbose_name}_{entity.id}.pdf',  # Archive name,
                        data=export_result.content,  # Data
                    )
                else:
                    assert isinstance(export_result, FileRef)
