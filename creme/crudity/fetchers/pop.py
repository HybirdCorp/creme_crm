...
class NEWPopFetcher(NEWCrudityFetcher):
    id = NEWCrudityFetcher.generate_id('crudity', 'pop')
    verbose_name = _('Emails (POP)')

    @classmethod
    def options_form(cls, **kwargs):
        from ..forms.fetchers.pop import PopFetcherOptionsForm

        return PopFetcherOptionsForm(**kwargs)

    def verbose_options(self):
        get_opt = self.options.get
        yield _('URL: {}').format(get_opt('url'))
        yield _('Username: {}').format(get_opt('username'))
        yield _('Password: ***')
        yield _('Port: {}').format(get_opt('port'))
        yield _('Use SSL: {}').format(_('yes') if get_opt('use_ssl') else _('no'))
