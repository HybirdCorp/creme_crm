...
if not documents.folder_model_is_custom():
    ...
    urlpatterns += [
        ...
        url(r'^folder/(?P<folder_id>\d+)/add/child[/]?$', folder.ChildFolderCreation.as_view(), name='documents__create_child_folder'),
        # url(r'^folder/edit/(?P<folder_id>\d+)[/]?$',      folder.edit,       name='documents__edit_folder'),
        url(r'^folder/edit/(?P<folder_id>\d+)[/]?$',      folder.FolderEdition.as_view(),       name='documents__edit_folder'),
...
if not documents.document_model_is_custom():
    ...
    urlpatterns += [
        ...
        url(r'^document/add_related/(?P<entity_id>\d+)[/]?', document.RelatedDocumentCreation.as_view(), name='documents__create_related_document'),
        # url(r'^document/edit/(?P<document_id>\d+)[/]?$',     document.edit,        name='documents__edit_document'),
        url(r'^document/edit/(?P<document_id>\d+)[/]?$',     document.DocumentEdition.as_view(),         name='documents__edit_document'),
