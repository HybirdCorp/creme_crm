...
urlpatterns = [
    ...
    *swap_manager.add_group(
        ...
        Swappable(re_path(r'^document/edit/(?P<document_id>\d+)[/]?$',     document.DocumentEdition.as_view(),         name='documents__edit_document'),           check_args=Swappable.INT_ID),
        Swappable(re_path(r'^document/(?P<document_id>\d+)[/]?$',          document.DocumentDetail.as_view(),          name='documents__view_document'),           check_args=Swappable.INT_ID),

        Swappable(re_path(r'^quickforms/from_widget/document/csv/add[/]?$', quick_forms.QuickDocumentCreation.as_view(), name='documents__create_document_from_widget')),
        Swappable(re_path(r'^quickforms/image[/]?$',                        quick_forms.QuickImageCreation.as_view(),    name='documents__create_image_popup')),
        app_name='documents',
    ).kept_patterns(),
]
