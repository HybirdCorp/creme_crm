...
class RelatedOpportunityCreateForm(OpportunityCreateForm):
    def __init__(self, *args, **kwargs):
        ...
        if description_f:
            description_f.initial = ugettext('Generated by the event «{}»').format(event)

        if not qs:
            fields['target'].help_text = ugettext('(The contact «{}» is not related to an organisation).').format(contact)
        else:
