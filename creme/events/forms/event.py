...
class RelatedOpportunityCreateForm(OpportunityCreateForm):
    def __init__(self, *args, **kwargs):
        ...
        fields['description'].initial = ugettext(u'Generated by the event "%s"') % event

        if not qs:
            fields['target'].help_text = ugettext(u'(The contact "%s" is not related to an organisation).') % contact
        else:
