...
class Populator(BasePopulator):
    ...
    def create_reports(self, rt_obj_emit_orga):
        ...
        from creme_core.models import EntityFilter, EntityFilterCondition, InstanceBlockConfigItem
        ...
        if Report.objects.filter(name=report_name, ct=opp_ct).exists():
            info('It seemns that a report "%s" already exists => do not recreate one.')
            return
        ...
        graph1 = create_graph(name=graph_name1, report=opp_report, abscissa='sales_phase',  ordinate='estimated_sales__sum', type=RGT_FK,    is_count=False, user=admin)
        graph2 = create_graph(name=graph_name2, report=opp_report, abscissa='closing_date', ordinate='estimated_sales__sum', type=RGT_RANGE, is_count=False, user=admin, days=90)

        #Create 2 instance block items for the 2 graphs ------------------------
        create_ibci = InstanceBlockConfigItem.objects.create
        ibci1 = create_ibci(entity=graph1, block_id=ReportGraphBlock.generate_id('creme_config', u"%s_" % graph1.id), verbose=u"%s - %s" % (graph1, _(u'None')), data='')
        ibci2 = create_ibci(entity=graph2, block_id=ReportGraphBlock.generate_id('creme_config', u"%s_" % graph2.id), verbose=u"%s - %s" % (graph2, _(u'None')), data='')

        BlockDetailviewLocation.create(block_id=ibci1.block_id, order=4, zone=BlockDetailviewLocation.RIGHT, model=Opportunity)
        BlockDetailviewLocation.create(block_id=ibci2.block_id, order=6, zone=BlockDetailviewLocation.RIGHT, model=Opportunity)
        BlockPortalLocation.create(app_name='opportunities', block_id=ibci1.block_id, order=1)
        BlockPortalLocation.create(app_name='opportunities', block_id=ibci2.block_id, order=2)
