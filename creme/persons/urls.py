...
urlpatterns = [
    ...
    re_path(r'^(?P<entity_id>\d+)/become_', include([
        ...
        re_path(r'^prospect[/]?$',          crud_relations.become_prospect, name='persons__become_prospect'),
        re_path(r'^suspect[/]?$',           crud_relations.become_suspect,  name='persons__become_suspect'),
        re_path(r'^inactive_customer[/]?$', crud_relations.become_inactive, name='persons__become_inactive_customer'),
        re_path(r'^supplier[/]?$',          crud_relations.become_supplier, name='persons__become_supplier'),
    ])),
]
...
urlpatterns += swap_manager.add_group(
    ...
    Swappable(re_path(r'^address/add/(?P<entity_id>\d+)[/]?$',          address.AddressCreation.as_view(),         name='persons__create_address'),          check_args=Swappable.INT_ID),
    Swappable(re_path(r'^address/add/billing/(?P<entity_id>\d+)[/]?$',  address.BillingAddressCreation.as_view(),  name='persons__create_billing_address'),  check_args=Swappable.INT_ID),
    Swappable(re_path(r'^address/add/shipping/(?P<entity_id>\d+)[/]?$', address.ShippingAddressCreation.as_view(), name='persons__create_shipping_address'), check_args=Swappable.INT_ID),
    Swappable(re_path(r'^address/edit/(?P<address_id>\d+)[/]?$',        address.AddressEdition.as_view(),          name='persons__edit_address'),            check_args=Swappable.INT_ID),
    app_name='persons',
).kept_patterns()
