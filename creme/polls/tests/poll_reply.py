...
class PollRepliesTestCase(_PollsTestCase):
    ...
    def test_create_from_pollform04(self):
        ...
        self.assertEqual([lines[1].question],
    ...
    def test_create_from_pollform06(self):
        ...
        name = 'Reply'
        reply_number = 5
        response = self.client.post(self._build_preply_from_pform_url(pform),
                                    data={'user':   user.id,
                                          'name':   name,
                                          'number': reply_number,
                                         }
    ...
    def test_listview(self):
        ...
        self.assertEqual(set([preply1, preply2]), set(preply_page.object_list))
    ...
    def test_section_tree(self):
        ...
        self.assertEqual([rline0, rsection1, rline1_1, rsection11, rline11_1, rline11_2, rsection2],
                         nodes
                        )
    ...
    def test_fillview_comment01(self):
        ...
        self.login()
        pform = PollForm.objects.create(user=self.user, name='Form#1')
        fline = PollFormLine.objects.create(pform=pform, type=PollLineType.COMMENT,
    ...
    def test_fillview_wizard_enumorstring01(self):
        ...
        self.assertFalse(self.rcondition.is_met(self.rline1))
        self.assertCurrentQuestion(self._fill(self.preply, {'answer_0': 1, 'answer_1': ''}),
                                   self.fline2
                                  )
    ...
    def test_fillview_wizard_enumorstring03(self):
        ...
        self.assertCurrentQuestion(self._fill(self.preply, {'answer_0': 0, 'answer_1': 'Pistachio'}),
                                   self.fline2
                                  )
    ...
    def test_stats_rline_boolean(self):
        ...
        self.assertEqual(set([(_('Yes'), 1), (_('No'), 0)]), set(rline.stats))

    def test_stats_rline_boolean_false(self):
        self.login()
        rline = self._build_rline_for_stat(PollLineType.BOOL, 0)
        self.assertEqual(set([(_('Yes'), 0), (_('No'), 1)]), set(rline.stats))
    ...
    def test_stats_rline_enum(self):
        ...
        self.assertEqual(set([('European', 1), ('African', 0)]), set(rline.stats))
    ...
    def test_stats_rline_multi_enum(self):
        ...
        self.assertEqual(set([('White', 0), ('Black', 1), ('Green', 1), ('Purple', 0)]),
                         set(rline.stats)
                        )
    ...
    def test_stats_rline_enum_or_string(self):
        ...
        self.assertEqual(set([('White', 0), ('Black', 0), ('Green', 0), ('Purple', 0), (_('Other'), 1)]),
                         set(rline.stats)
                        )
    ...
    def test_stats_rline_enum_or_string_custom(self):
        ...
        self.assertEqual(set([('White', 0), ('Black', 1), ('Green', 0), ('Purple', 0), (_('Other'), 0)]),
                         set(rline.stats)
                        )
    ...
    def test_stats_tree(self):
        ...
        self.assertEqual(set([(answer_2_1, 2, round((2.0 * 100.0) / 3.0, 2)),
                              (answer_2_4, 1, round((1.0 * 100.0) / 3.0, 2)),
                             ]),
                         set(stats2)
                        )
        self.assertEqual(set([(u'European', 1, round((1.0 * 100.0) / 3.0, 2)),
                              (u'African',  2, round((2.0 * 100.0) / 3.0, 2)),
                             ]),
                         set(stats3)
                        )
        self.assertEqual(set([(u'White',  3, round((3.0 * 100.0) / 6.0, 2)),
        ...
        self.assertEqual(set([(u'White',   1, round((1.0 * 100.0) / 3.0, 2)),
                              (u'Black',   0, 0.0),
                              (u'Green',   0, 0.0),
                              (u'Purple',  0, 0.0),
