...
EFC_VALUE_FMT = '{"operator": 1, "values": ["%s"]}'
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm['commercial.CommercialApproach'].objects.filter(entity_content_type__in=old_ctypes) \
        ...
        TYPE_MAP = {meeting_ct.id: meeting_filter,
                    pcall_ct.id:   pcall_filter,
                    task_ct.id:    task_filter,
                   }

        for name in ('ActObjective', 'ActObjectivePatternComponent'):
            for instance in orm['commercial.%s' % name].objects.filter(ctype__in=old_ctypes):
                if instance.filter:
                    pass # work done by 0005_v1_3__migrate_n_clean_old_activities_n_related
                else:
                    instance.filter = TYPE_MAP[instance.ctype_id]

                instance.ctype = activity_ct
                instance.save()
