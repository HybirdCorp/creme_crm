...
@skipIfCustomStrategy
class StrategyViewsTestCase(CommercialBaseTestCase):
...
@skipIfCustomStrategy
class SegmentDescriptionViewsTestCase(BrickTestCaseMixin, CommercialBaseTestCase):
    @staticmethod
    def _build_edit_segment_desc_url(segment_desc):
    ...
    def test_creation(self):
        ...
        url = self._build_add_segment_desc_url(strategy)
    ...
    def test_creation__name_collision(self):
        user = self.login_as_root_and_get()
        strategy = Strategy.objects.create(user=user, name='Strat#1')
    ...
    def test_creation__property_collision(self):
        ...
        response = self.assertPOST200(
            self._build_add_segment_desc_url(strategy), data={'name': name},
        )
        self.assertFormError(
            self.get_form_or_fail(response),
            field='name',
    ...
    def test_segment_linking(self):
        user = self.login_as_root_and_get()
        create_strategy = partial(Strategy.objects.create, user=user)
        strategy1 = create_strategy(name='Strat#1')
        industry = self._create_segment_desc(strategy1, 'Industry').segment
        self.assertEqual(1, strategy1.segment_info.count())

        strategy2 = create_strategy(name='Strat#2')
        self.assertFalse(0, strategy2.segment_info.exists())

        url = self._build_link_segment_url(strategy2)
        context = self.assertGET200(url).context
        self.assertEqual(
            _('New market segment for «{entity}»').format(entity=strategy2),
            context.get('title'),
        )
        self.assertEqual(
        ...
        description = self.get_alone_element(strategy2.segment_info.all())
    ...
    def test_edition(self):
        ...
        url = self._build_edit_segment_desc_url(segment_desc)
    ...
    def test_edition__no_name_change(self):
        ...
        response = self.client.post(
            self._build_edit_segment_desc_url(segment_desc),
    ...
    def test_edition__no_ptype(self):
...
@skipIfCustomStrategy
class AssetViewsTestCase(BrickTestCaseMixin, CommercialBaseTestCase):
    def test_creation(self):
        user = self.login_as_root_and_get()
        strategy = Strategy.objects.create(user=user, name='Strat#1')
    ...
    @skipIfCustomOrganisation
    def test_set_score__several_segments(self):
        user = self.login_as_root_and_get()
        strategy = Strategy.objects.create(user=user, name='Strat#1')

        segment_desc1 = self._create_segment_desc(strategy, 'Industry')
        segment_desc2 = self._create_segment_desc(strategy, 'People')
        ...
        self.assertEqual(1, strategy.get_asset_score(orga, asset01, segment_desc1))
        self.assertEqual(1, strategy.get_asset_score(orga, asset01, segment_desc2))
        self.assertEqual(1, strategy.get_asset_score(orga, asset02, segment_desc1))
        self.assertEqual(1, strategy.get_asset_score(orga, asset02, segment_desc2))
        ...
        score21 = 3
        score22 = 2
        self._set_asset_score(strategy, orga, asset01, segment_desc1, score11)
        self._set_asset_score(strategy, orga, asset01, segment_desc2, score12)
        self._set_asset_score(strategy, orga, asset02, segment_desc1, score21)
        self._set_asset_score(strategy, orga, asset02, segment_desc2, score22)

        strategy = self.refresh(strategy)  # (cache....)
        self.assertEqual(score11, strategy.get_asset_score(orga, asset01, segment_desc1))
        self.assertEqual(score12, strategy.get_asset_score(orga, asset01, segment_desc2))
        self.assertEqual(score21, strategy.get_asset_score(orga, asset02, segment_desc1))
        self.assertEqual(score22, strategy.get_asset_score(orga, asset02, segment_desc2))
...
@skipIfCustomStrategy
class CharmViewsTestCase(BrickTestCaseMixin, CommercialBaseTestCase):
    def test_creation(self):
        user = self.login_as_root_and_get()
        strategy = Strategy.objects.create(user=user, name='Strat#1')
...
@skipIfCustomStrategy
class EvaluatedOrganisationViewsTestCase(BrickTestCaseMixin, CommercialBaseTestCase):
    @skipIfCustomOrganisation
    ...
    @skipIfCustomOrganisation
    def test_detail_view__unrelated(self):
    ...
    @skipIfCustomOrganisation
    def test_detail_view__regular_user(self):
    ...
    @skipIfCustomOrganisation
    def test_detail_view__view_perms__strategy(self):
        "View credentials for Strategy."
        user = self.login_as_standard(allowed_apps=['commercial', 'persons'])
    ...
    @skipIfCustomOrganisation
    def test_detail_view__view_perms__organisation(self):
        "View credentials for Organisation."
        user = self.login_as_standard(allowed_apps=['commercial', 'persons'])
        self.add_credentials(user.role, own=['VIEW'])
...
@skipIfCustomStrategy
class BricksReloadingTestCase(BrickTestCaseMixin, CommercialBaseTestCase):
    @skipIfCustomOrganisation
    def test_assets_matrix(self):
        user = self.login_as_root_and_get()
        strategy = Strategy.objects.create(user=user, name='Strat#1')
        segment_desc = self._create_segment_desc(strategy, 'Industry')
    ...
    def test_assets_matrix__no_app_perm(self):
    ...
    @skipIfCustomOrganisation
    def test_assets_charms_matrix(self):
        user = self.login_as_root_and_get()
        strategy = Strategy.objects.create(user=user, name='Strat#1')
        segment_desc = self._create_segment_desc(strategy, 'Industry')
        asset = CommercialAsset.objects.create(name='Size', strategy=strategy)
