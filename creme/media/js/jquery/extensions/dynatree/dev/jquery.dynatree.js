		...
		for(var i=0; i<this.childList.length; i++ ) {
			...
			if( fxDuration ) {
				...
				if( bExpand != $child.is(':visible') )
					$child.animate(opts.fx, fxDuration, "swing", opts.onAfterExpand );
			} else {
				if( bExpand )
					$child.show( opts.onAfterExpand );
				else
					$child.hide( opts.onAfterExpand ); // TODO: this seems to be slow, when called the first time for an element
			}
		}
...
$.ui.dynatree.defaults = {
	...
	onAfterExpand: null, // Callback(dtnode) when a node has finish to expand/collapse.
