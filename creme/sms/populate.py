...
class Populator(BasePopulator):
    ...
    def populate(self):
        ...
        if not BlockDetailviewLocation.config_exists(SMSCampaign): # NB: no straightforward way to test that this populate script has not been already run
            ...
            create_bdl(block_id=bricks.SendingsBrick.id_,          order=2,   zone=TOP,   model=SMSCampaign)
            create_bdl(block_id=core_bricks.CustomFieldsBrick.id_, order=40,  zone=LEFT,  model=SMSCampaign)
            create_bdl(block_id=bricks.MessagingListsBlock.id_,    order=50,  zone=LEFT,  model=SMSCampaign)
            create_bdl(block_id=core_bricks.PropertiesBrick.id_,   order=450, zone=LEFT,  model=SMSCampaign)
            create_bdl(block_id=core_bricks.RelationsBrick.id_,    order=500, zone=LEFT,  model=SMSCampaign)
            create_bdl(block_id=core_bricks.HistoryBrick.id_,      order=20,  zone=RIGHT, model=SMSCampaign)

            BlockDetailviewLocation.create_4_model_brick(order=5,             zone=LEFT,  model=MessagingList)
            create_bdl(block_id=core_bricks.CustomFieldsBrick.id_, order=40,  zone=LEFT,  model=MessagingList)
            create_bdl(block_id=bricks.RecipientsBrick.id_,        order=50,  zone=LEFT,  model=MessagingList)
            create_bdl(block_id=bricks.ContactsBrick.id_,          order=55,  zone=LEFT,  model=MessagingList)
            create_bdl(block_id=core_bricks.PropertiesBrick.id_,   order=450, zone=LEFT,  model=MessagingList)
            create_bdl(block_id=core_bricks.RelationsBrick.id_,    order=500, zone=LEFT,  model=MessagingList)
            create_bdl(block_id=core_bricks.HistoryBrick.id_,      order=20,  zone=RIGHT, model=MessagingList)
            ...
            if apps.is_installed('creme.documents'):
                ...
                create_bdl(block_id=LinkedDocsBrick.id_, order=600, zone=RIGHT, model=SMSCampaign)
