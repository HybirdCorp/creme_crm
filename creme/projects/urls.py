...
urlpatterns += swap_manager.add_group(
    ...
    Swappable(re_path(r'^project/(?P<project_id>\d+)[/]?$',      project.ProjectDetail.as_view(),   name='projects__view_project'), check_args=Swappable.INT_ID),
    app_name='projects',
).kept_patterns()

urlpatterns += swap_manager.add_group(
    task_model_is_custom,
    Swappable(re_path(r'^project/(?P<project_id>\d+)/task/add[/]?$', task.TaskCreation.as_view(),     name='projects__create_task'),     check_args=Swappable.INT_ID),
    Swappable(re_path(r'^task/edit/(?P<task_id>\d+)[/]?$',           task.TaskEdition.as_view(),      name='projects__edit_task'),       check_args=Swappable.INT_ID),
    Swappable(re_path(r'^task/edit/(?P<task_id>\d+)/popup[/]?$',     task.TaskEditionPopup.as_view(), name='projects__edit_task_popup'), check_args=Swappable.INT_ID),
    Swappable(re_path(r'^task/(?P<task_id>\d+)[/]?$',                task.TaskDetail.as_view(),       name='projects__view_task'),       check_args=Swappable.INT_ID),
    app_name='projects',
).kept_patterns()
